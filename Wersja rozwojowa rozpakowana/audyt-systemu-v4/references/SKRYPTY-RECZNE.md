...
| `check_wartosci_prawne.py` | **T28 — NIE jest skryptem ręcznym.** Offline, deterministyczny, bez sieci — wchodzi do orkiestratora jak T26. Wpisany tutaj wyłącznie po to, żeby nie szukać go w tej tabeli: uruchamiać automatycznie przy każdym przebiegu regresji, a ręcznie tylko przy dopisywaniu pozycji do rejestru W1 (wtedy obowiązkowo `--selftest`). | Orkiestrator; ręcznie przy zmianie rejestru W1. |
