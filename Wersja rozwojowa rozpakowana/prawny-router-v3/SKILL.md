...
changelog:
  # ⛔ ZASADA 15: to pole jest SKRÓTEM bieżącej wersji, nie historią.
  # Pełna historia (w tym 63 linie przeniesione stąd 2026-08-24, F-126):
  #   view /mnt/skills/user/prawny-router-v3/references/CHANGELOG.md
  - "3.23 (2026-08-24, flaga F-126): historia wersji wyniesiona z pola YAML
    i z sekcji ## CHANGELOG w korpusie do references/CHANGELOG.md — jedna
    lokalizacja kanoniczna. Treść przeniesiona 1:1, bez rekonstrukcji.
    Opis: audyt-systemu-v4/references/AUDIT-JOURNAL.md, AUDYT-2026-08-24."
---
...
KROK 3A → [ŚLAD ROUTINGU — OBOWIĄZKOWY, dodano 2026-08-23f, F-119]
          ...
          ```
          ⛔ Powód: trzy niezależne testy zewnętrzne (TEST1 sekcja 7, TEST2
          B-01, TEST3 CX-01) nie mogły rozstrzygnąć, czy router został
          faktycznie WCZYTANY (`view` wykonany), czy tylko NAZWANY w treści
          odpowiedzi bez realnego wywołania narzędzia. Ten blok czyni ten
          stan JAWNYM zamiast milczącym.
          ...
          ⛔⛔ OGRANICZENIE ZNANE (dodano 2026-08-23f, po pytaniu użytkownika
          — NIE ukrywać tej luki): to pole jest SAMO-RAPORTEM. Jeśli
          narzędzia SĄ dostępne, ale wywołanie `view` zostało POMINIĘTE
          z innego powodu niż brak dostępu, nic w tym kroku nie wymusza
          poprawnej wartości `NIE` — model, który już raz pominął
          wczytanie, może z tego samego powodu błędnie zadeklarować `TAK`.
          Pole jest wiarygodne WYŁĄCZNIE gdy `NIE` wynika z TWARDEGO
          FAKTU: próba `view` zwróciła błąd narzędzia (środowisko bez
          dostępu do plików). W KAŻDYM INNYM przypadku deklaracja
          `ROUTER-WCZYTANY: TAK` musi być poparta rzeczywiście wykonanym
          wywołaniem w TEJ odpowiedzi — sam zapis w polu tego nie
          zastępuje i nie udowadnia. Ten blok NIE rozwiązuje problemu
          fasady, tylko czyni STAN JAWNYM w przypadku, gdy brak dostępu
          jest faktem technicznym możliwym do stwierdzenia — nie w
          przypadku, gdy model błądzi mimo posiadania dostępu. Miara
          skuteczności wobec drugiego scenariusza: F-113 zakres (e).
          ⛔ NIE polegaj wyłącznie na tym polu — `references/SELF-CHECK.md`
          zawiera KONTROLĘ NA WYJŚCIU (wzorem DOMAIN-LOCK): sprawdzenie,
          czy gotowa odpowiedź nosi ślady użycia PRIMARY bez odpowiadającego
          wywołania w tej odpowiedzi. To druga, niezależna linia — deklaracja
          z tego kroku i obserwacja na wyjściu muszą być ZGODNE.
...
*(Wpisy 3.13 … 3.9 przeniesione stąd 1:1 do `references/CHANGELOG.md`
