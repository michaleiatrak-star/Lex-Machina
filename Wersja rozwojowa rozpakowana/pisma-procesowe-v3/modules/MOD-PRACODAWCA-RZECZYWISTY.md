...
T5: wydzielenie/utworzenie NOWEGO podmiotu o tym samym/podobnym profilu
    działalności, obsługującego TYCH SAMYCH klientów co poprzedni
    pracodawca — NIEZALEŻNIE od tego, czy właściciel/wspólnicy są
    identyczni, częściowo pokrywający się, czy całkowicie inni (dodane
    2026-08-04, na żądanie użytkownika — patrz sekcja "WYDZIELENIE
    CZĘŚCI FIRMY" niżej)
...
## ⭐ WYDZIELENIE CZĘŚCI FIRMY / SPIN-OFF (dodane 2026-08-04, na
żądanie użytkownika — odpowiedź: TAK, ta sama doktryna obejmuje ten
scenariusz)

```
⭐⭐ KLUCZOWA ODPOWIEDŹ NA PYTANIE UŻYTKOWNIKA: doktryna
"pracodawcy rzeczywistego"/przejścia zakładu pracy OBEJMUJE
scenariusz wydzielenia nowej firmy o identycznym profilu,
obsługującej tych samych klientów — NIEZALEŻNIE OD STRUKTURY
WŁASNOŚCIOWEJ (inny właściciel, jeden ze wspólników, lub BRAK
zmiany właścicieli w ogóle) — decydują OKOLICZNOŚCI FAKTYCZNE, nie
formalna struktura kapitałowa

⭐ FUNDAMENT PRAWNY (potwierdzony wielokrotnie w orzecznictwie SN i
  doktrynie akademickiej, w tym implementacja dyrektywy 2001/23/WE):
  → Art. 23¹ KP działa Z MOCY PRAWA (ex lege) — do przejścia zakładu
    pracy MOŻE DOJŚĆ "bez woli stron, a NAWET bez ich wiedzy" —
    decydujące są WYŁĄCZNIE okoliczności faktyczne (przejęcie
    substratu majątkowego LUB zadań), NIE formalna czynność prawna
  → ⭐⭐ Art. 23¹ KP jest PRZEPISEM BEZWZGLĘDNIE OBOWIĄZUJĄCYM
    (ius cogens) — "jakiekolwiek uzgodnienia pomiędzy pracodawcami,
    które prowadziłyby do OBEJŚCIA tego przepisu, SĄ NIEWAŻNE" —
    NIE MOŻNA umownie "wyłączyć" ochrony pracownika przez
    odpowiednie zaplanowanie struktury spółek
  → DATA przejścia = data FAKTYCZNEGO przejęcia majątku/zadań przez
    nowy podmiot — NIE data formalnej czynności prawnej (np. wpisu
    do KRS, aktu notarialnego) — potwierdzone konsekwentnie w
    orzecznictwie SN

⭐⭐ TEST "SPIJKERS" (z orzecznictwa TSUE, C-24/85, implementowany
  przez polskie sądy) — WIELOCZYNNIKOWA, CAŁOŚCIOWA ocena, ŻADEN
  POJEDYNCZY czynnik NIE jest samodzielnie decydujący:
  1) Rodzaj przedsiębiorstwa/zakładu
  2) Czy doszło do przeniesienia SKŁADNIKÓW MATERIALNYCH (budynki,
     sprzęt, wyposażenie)
  3) WARTOŚĆ składników NIEMATERIALNYCH w chwili przejęcia
  4) ⭐⭐ CZY NOWY PODMIOT PRZEJĄŁ WIĘKSZOŚĆ PRACOWNIKÓW
  5) ⭐⭐ CZY PRZEJĘTO KLIENTELĘ (KLUCZOWY czynnik DLA PYTANIA
     UŻYTKOWNIKA — "ci sami klienci" to WPROST jeden z oficjalnych
     kryteriów testu, NIE poboczna okoliczność)
  6) STOPIEŃ PODOBIEŃSTWA działalności prowadzonej przed i po
     przejęciu
  7) OKRES ewentualnej przerwy w działalności (jeśli była)

  ⭐ WNIOSEK: scenariusz z pytania użytkownika (identyczny profil +
  ci sami klienci) TRAFIA WPROST w PUNKT 5 i 6 testu — TO SILNA
  PRZESŁANKA przemawiająca za uznaniem przejścia zakładu pracy,
  NIEZALEŻNIE od tego, co dzieje się ze strukturą właścicielską
  (punkt 4 — przejęcie pracowników — TEŻ typowo współwystępuje w
  takich scenariuszach, dodatkowo wzmacniając ocenę)

STRUKTURA WŁASNOŚCIOWA NIE MA ZNACZENIA DLA SAMEJ OCENY, CZY DOSZŁO
  DO PRZEJŚCIA — bo art. 23¹ KP dotyczy STOSUNKU PRACY (relacji
  pracownik-pracodawca jako PODMIOT PRAWNY), NIE struktury
  kapitałowej właścicieli TYCH podmiotów:
  → Scenariusz (a) — INNY właściciel nowej spółki: KLASYCZNY
    przypadek przejścia zakładu pracy (sprzedaż/przekazanie
    zorganizowanej części przedsiębiorstwa)
  → Scenariusz (b) — JEDEN ZE WSPÓLNIKÓW poprzedniej spółki
    zakłada nową: ta sama doktryna DZIAŁA, ZE WZMOCNIONYM
    argumentem o ZAMIARZE obejścia prawa (art. 58 §1 KC w zw. z art.
    300 KP — patrz WARSTWA 2 protokołu R1-R5 wyżej w tym module) —
    bliskość personalna między "starym" a "nowym" podmiotem jest
    DODATKOWĄ poszlaką świadomego planowania, nie okolicznością
    łagodzącą
  → Scenariusz (c) — BRAK zmiany właścicieli (ten sam
    właściciel/wspólnicy zakładają formalnie odrębny podmiot):
    NAJSILNIEJSZY scenariusz dla argumentacji pracownika — TRUDNO o
    "biznesowe" uzasadnienie inne niż chęć zresetowania limitów
    umów terminowych/uniknięcia stażu pracy/inne korzyści kadrowe —
    ANALOGICZNIE do argumentu "venire contra factum proprium" (art.
    8 KP, patrz WARSTWA 3 wyżej w tym module)

⚠️ WAŻNE ZASTRZEŻENIE — GRANICA DOKTRYNY (orzecznictwo NIE JEST
  jednostronnie "propracownicze", wymaga RZECZYWISTEGO substratu):
  → SA Katowice (III AUa 2400/17, 5.04.2018): SAM transfer
    PRACOWNIKÓW, BEZ przejścia wykorzystywanego przez nich
    "substratu majątkowego" (infrastruktury), NIE WYPEŁNIA normy
    art. 23¹ §1 KP — jeśli działalność WYMAGA infrastruktury
    majątkowej, a NOWY podmiot jej NIE PRZEJĄŁ, sam transfer zadań
    MOŻE NIE WYSTARCZYĆ
  → SA Lublin (III AUa 536/19, 4.12.2019): sporządzenie NOWYCH umów
    na piśmie i wypłata wynagrodzenia PRZEZ nowy podmiot SAME W
    SOBIE NIE PRZESĄDZAJĄ o faktycznym przejęciu — liczy się
    SUBSTANCJA, nie forma dokumentacji
  → OSTRZEŻENIE PRZED "POZORNYM OUTSOURCINGIEM PRACOWNICZYM": gdy
    celem jest GŁÓWNIE uniknięcie składek ZUS/inne korzyści
    formalne, BEZ faktycznego przekazania realnych zadań/majątku —
    orzecznictwo MOŻE uznać to za PRZEJŚCIE FIKCYJNE mimo
    formalnego przejęcia — RYZYKO działa W OBIE STRONY (zarówno
    ryzyko nieuznania przejścia mimo pozorów, jak i ryzyko uznania
    przejścia mimo braku formalności)

⚠️ ODRĘBNY, FORMALNY MECHANIZM KORPORACYJNY (dla porównania — NIE
  WYMAGANY do zastosowania art. 23¹, ale WARTO sprawdzić czy
  wystąpił): "podział przez wydzielenie" (art. 529 §1 pkt 4 KSH) —
  sformalizowana procedura przeniesienia części majątku spółki
  dzielonej na spółkę NOWO ZAWIĄZANĄ, z planem podziału (art. 534
  KSH), gdzie spółka nowo zawiązana WSTĘPUJE w prawa i obowiązki z
  dniem wydzielenia — JEŚLI TAKA formalna procedura WYSTĄPIŁA, jest
  to DODATKOWY, silny dowód przejścia (obok samych faktów
  biznesowych) — ALE jej BRAK NIE WYKLUCZA zastosowania art. 23¹ KP,
  który działa NIEZALEŻNIE od tego, czy zastosowano formalną
  procedurę KSH, czy nie

PRAKTYCZNA STRATEGIA DOWODOWA (rozszerzenie protokołu R1-R5 dla
  scenariusza wydzielenia): DODATKOWO do standardowych dowodów
  (WARSTWA 4), zbierz: (a) listę/próbkę klientów obsługiwanych PRZED
  i PO wydzieleniu — im WIĘKSZE pokrycie, tym silniejszy dowód, (b)
  dowody na PRZEJĘCIE know-how/metod pracy/procedur, (c) dowody
  ciągłości ADRESU/lokalu/numerów telefonów/strony internetowej —
  klienci "nie zauważający" zmiany dostawcy usługi to SILNA poszlaka
  ciągłości tożsamości przedsiębiorstwa, (d) jeśli dostępne: KRS
  nowego podmiotu pod kątem dat rejestracji względem dat
  "zniknięcia" zleceń u poprzedniego podmiotu (bliskość czasowa
  wzmacnia podejrzenie planowania)

Potwierdzone w 7+ zgodnych źródłach, w tym publikacja akademicka
(czasopisma.uwm.edu.pl, z bezpośrednim odesłaniem do wyroku TSUE
Spijkers C-24/85), rp.pl [x2], Russell Bedford [x2], strefafirmy.pl,
poradnikprzedsiebiorcy.pl, infor.pl, oraz orzecznictwo SN i sądów
apelacyjnych cytowane wyżej.
```
