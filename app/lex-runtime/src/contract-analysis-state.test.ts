...
function close(
  state: ReturnType<
    typeof createContractAnalysisState
  >,
  checkpoint: Parameters<
    typeof completeContractCheckpoint
  >[1]
) {
...
describe(
  ...
  () => {
    ...
    it(
      "routes redaction, draft and supplement through the generation branch",
      () => {
        for (
          const mode
          of [
    ...
    it(
      "allows objective N/A only for intake gap and party-identity checks",
      () => {
        let state =
          createContractAnalysisState(
            CASE_ID,
        ...
        state =
          markContractCheckpointNotApplicable(
            state,
            "AU-GAP",
            "Brak pól wymagających uzupełnienia po intake.",
            [
        ...
        state =
          markContractCheckpointNotApplicable(
            state,
            "AU-POV",
            "Dokument nie zawiera danych podmiotu do odrębnej weryfikacji.",
            [
    ...
    it(
      "blocks out-of-order and unaudited transitions",
      () => {
        const state =
          createContractAnalysisState(
            CASE_ID,
        ...
        expect(() =>
          ...
          "CONTRACT_CHECKPOINT_OUT_OF_ORDER"
        );
    ...
    it(
      "never permits finalization to be marked N/A",
      () => {
        const state =
          createContractAnalysisState(
            CASE_ID,
        ...
        expect(() =>
          markContractCheckpointNotApplicable(
            state,
            "AU-HYBRID",
            "Użytkownik chce pominąć walidację.",
            [
