  ...
  const result:
    SessionExecutionResponse = {
      ...
      primarySkill:
        "analiza-sadowa-v6",
      answer:
        "TA TREŚĆ NIE MOŻE TRAFIĆ DO AUDYTU",
      documentCitations: [
        {
          citationId:
            "docref_1",
...
describe(
  ...
  () => {
    it(
      "persists only privacy-bounded execution evidence",
      () => {
        const bytes =
          buildWorkflowAuditArtifact({
            caseId:
    ...
    it(
      "refuses to build an artifact for a blocked or mismatched workflow",
      () => {
        const blocked =
          resultFixture();
    ...
    it(
      "rejects tampered stored payloads",
      () => {
        const bytes =
          buildWorkflowAuditArtifact({
            caseId:
