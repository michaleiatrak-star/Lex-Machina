...
describe(
  ...
  () => {
    it(
      "classifies a message without a legal matter so no legal skills are loaded",
      async () => {
        const registry =
          fixture();
        const setup =
        ...
        expect(
          result.decision.legal
        ).toBe(false);
        expect(
          result.decision.executionSkills
        ).toEqual([]);
        expect(
          result.decision.workflowExecutionSkill
        ).toBeNull();
        expect(
    ...
    it(
      "does not call a local model to route a trivial chat command",
      async () => {
        const setup =
          router(
            fixture(),
    ...
    it(
      "routes local models from the compact catalog without the central routing map",
      async () => {
        const setup =
          router(
            fixture(),
        ...
        expect(
          setup.adapter.calls[0]?.systemPrompt
        ).not.toContain(
    ...
    it(
      "routes a short local legal question to a domain only, with a bounded answer",
      async () => {
        const setup =
          router(
            fixture(),
