...
const historical = bundle.find(
  (item) => item.temporalMode === "HISTORICAL"
);
const pdf = bundle.find(
  (item) => item.sourceFormat === "PDF"
);
const quote = bundle.find(
  (item) => item.caseScope === "EXACT_QUOTE"
);
const supported = bundle.find(
