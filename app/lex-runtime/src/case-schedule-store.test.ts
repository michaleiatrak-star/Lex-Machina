...
describe(
  ...
  () => {
    it(
      "stores, reads and rekeys a matter schedule",
      async () => {
        const root =
          await mkdtemp(
            path.join(
    ...
    it(
      "returns an empty schedule before the first event is stored",
      async () => {
        const root =
          await mkdtemp(
            path.join(
    ...
    it("keeps contacts and events apart in one encrypted file", async () => {
      ...
      expect(
        await store.listContacts({ caseId, caseDataKey: newKey, keyVersion: 2 })
      ).toEqual([contact]);
    });
  }
);
