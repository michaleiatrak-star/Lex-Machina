import type { IngestedPage, OcrCorrection, OcrLine } from "./document-ingestion.js";

/**
 * OCR post-correction with the local model ("z lokalnym AI"). The model does
 * not read the image (local models are text-only). It reads whole passages
 * (several lines, full sentences) and returns a list of fixes, never a
 * rewritten text: the document's own wording and grammar stay as written,
 * only what the OCR got wrong is fixed.
 *
 * A fix covers 1-3 adjacent words of one badly read line (or a word hyphenated
 * over two lines) and is taken only when:
 * - every resulting word is in the SGJP dictionary;
 * - it is a small edit: diacritics, a split or merged word, or 1-2 characters
 *   (1 in a capitalized word, which may be a name);
 * - a word the dictionary knows changes only by diacritics ("sad" -> "sąd");
 * - numbers, dates, amounts and identifiers are untouched and no "nie" is
 *   added or removed.
 * Stray OCR symbols (|, ~, ¦, • ...) may be removed when the rest of the
 * fragment stays the same. Before the model, the text is cleaned without it:
 * ligatures, invisible characters, odd spaces, Cyrillic/Greek look-alike
 * letters inside Latin words, Unicode composition.
 * The original words are kept in page.corrections.
 */

export type AskLocalModel = (system: string, content: string) => Promise<string>;
export type KnownWords = (words: string[]) => Promise<boolean[]>;
export type OcrFixCheck = (item: string, done: number, total: number) => void;
 ...
 * the visible changes.
 */
export function normalizeOcrLine(text: string): { text: string; fixes: Array<{ from: string; to: string }> } {
  ...
  result = result.replace(/[\p{L}]+/gu, (word) => {
    ...
    if (!foreign.length || !latin.length) return word;
    const fixed = [...word].map((char) => LOOKALIKES[char] ?? char).join("");
    if (/[^\p{Script=Latin}]/u.test(fixed)) return word;
...
function withoutArtifacts(value: string): string {
...
function isCapital(value: string): boolean {
  const first = value[0] ?? "";
...
export function candidateWord(word: string): boolean {
...
function negations(value: string): number {
 ...
 */
export function acceptableFix(rawFrom: string, to: string): boolean {
  // Stray symbols may only be dropped; what remains follows the word rules.
  const from = withoutArtifacts(rawFrom);
  if (rawFrom !== from && from === to.trim()) return words(to).length > 0 && !HAS_ARTIFACT.test(to);
  const fromWords = words(from);
  const toWords = words(to);
  if (!fromWords.length || !toWords.length || fromWords.length > MAX_FIX_WORDS || toWords.length > MAX_FIX_WORDS) return false;
  if (from === to || !/^\p{L}+(?:[-'’]\p{L}+)*(?:\s+\p{L}+(?:[-'’]\p{L}+)*)*$/u.test(to)) return false;
  // Digits in "from" only as look-alikes inside a word.
  if (fromWords.some((word) => /\p{N}/u.test(word) && !candidateWord(word))) return false;
  if (isCapital(from) !== isCapital(to) || negations(from) !== negations(to)) return false;
  const a = from.replace(/[\s-]+/g, "");
  const b = to.replace(/[\s-]+/g, "");
  if (stripDiacritics(a).toLocaleLowerCase("pl") === stripDiacritics(b).toLocaleLowerCase("pl")) return true;
...
function parseFixes(raw: string): Array<{ line: number; from: string; to: string }> {
  ...
  try {
    ...
    return parsed.flatMap((item) => {
      const record = item as { line?: unknown; id?: unknown; from?: unknown; to?: unknown };
...
export class LocalOcrCorrector {
  constructor(
    private readonly model: () => AskLocalModel | null,
    private readonly knownWords: KnownWords
  ) {}

  /** The page with fixes applied (original words in page.corrections); unchanged when nothing to fix. */
  async correct(page: IngestedPage, onCheck?: OcrFixCheck): Promise<IngestedPage> {
    const original = page.lines;
    if (page.source !== "OCR" || !original?.length) return page;
    const ask = this.model();
    if (!ask) throw new Error("LOCAL_PRIVACY_MODEL_NOT_READY");

    const corrections: OcrCorrection[] = [];
    const lines: OcrLine[] = original.map((line, index) => {
    ...
    const suspicious = new Set(
      lines
        ...
        .filter(({ index, marked }) =>
          HAS_ARTIFACT.test(lines[index]!.text) ||
          (lowScore(index)
    ...
    if (!suspicious.size) return this.result(page, original, lines, corrections);

    const toSend = passages(lines).filter((group) => group.some((index) => suspicious.has(index)));
    const batches: number[][][] = [];
    for (const group of toSend) {
      const last = batches.at(-1);
      if (last && last.flat().length + group.length <= BATCH_LINES) last.push(group);
    ...
    for (const batch of batches) {
      onCheck?.(
        batch.flat().filter((index) => suspicious.has(index)).flatMap((index) => words(lines[index]!.text).filter((word) => unknown.has(word))).join(", ") ||
          `linie ${batch.flat().map((index) => index + 1).join(", ")}`,
        done,
        total
      );
      const content = batch
      ...
      for (const fix of parseFixes(await ask(PROMPT, content))) {
        ...
        if (!sent.has(index) || !suspicious.has(index)) continue;
    ...
    const accepted = proposals.filter((fix) =>
      words(fix.to).every((word) => valid.has(word) || words(fix.from).includes(word))
    );
    ...
    for (const fix of accepted) {
      ...
      if (fix.next !== undefined) {
        ...
        if (!next || !line.text.trimEnd().endsWith(head) || !next.text.startsWith(fix.next)) continue;
  ...
  private result(page: IngestedPage, original: OcrLine[], fixed: OcrLine[], corrections: OcrCorrection[]): IngestedPage {
    if (!corrections.length) return page;
    let text = "";
    let cursor = 0;
  ...
  private validate(
    ...
    if (!fragmentPattern(from).test(line) || !acceptableFix(from, to)) return null;
    // Only stray symbols dropped: nothing else to check.
    if (withoutArtifacts(from) === to && from !== to) return { line: index, from, to };
    const fromWords = words(from);
    const toWords = words(to);
    ...
    if (!hasUnknown && !reshaped) {
      ...
      if (!lowScore || !fromWords.every((word) => known.has(word) || !candidateWord(word))) return null;
