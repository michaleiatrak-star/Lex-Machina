import type { IngestedPage, OcrCorrection } from "./document-ingestion.js";

/**
 * OCR post-correction with the local model ("z lokalnym AI"). The model does
 * not read the image (local models are text-only): it only proposes fixes
 * for words the SGJP dictionary does not know, in badly read lines, from the
 * sentence around them. A fix is taken only when it is a small edit (or a
 * diacritics fix), gives a dictionary word and keeps the capitalization;
 * numbers, dates, amounts and identifiers are never touched.
 */
...
export function acceptableFix(from: string, to: string): boolean {
  if (!to || from === to || /[\p{N}\s]/u.test(to) || !/^\p{L}+(?:[-'’]\p{L}+)*$/u.test(to)) return false;
  const capital = (value: string) => value[0] === value[0]!.toLocaleUpperCase("pl") && value[0] !== value[0]!.toLocaleLowerCase("pl");
  if (capital(from) !== capital(to)) return false;
  if (stripDiacritics(from).toLocaleLowerCase("pl") === stripDiacritics(to).toLocaleLowerCase("pl")) return true;
...
function parseFixes(raw: string): Array<{ id: number; from: string; to: string }> {
...
export class LocalOcrCorrector {
  ...
  async correct(page: IngestedPage, onCheck?: OcrFixCheck): Promise<IngestedPage> {
    ...
    if (page.source !== "OCR" || !lines?.length) return page;
    ...
    if (!suspicious.length) return page;

    const proposals: Array<{ line: number; from: string; to: string }> = [];
    for (let offset = 0; offset < suspicious.length; offset += BATCH_LINES) {
      ...
      for (const fix of parseFixes(await ask(PROMPT, content))) {
        ...
        if (!line || !line.marked.includes(fix.from) || !acceptableFix(fix.from, fix.to)) continue;
    ...
    if (!accepted.length) return page;
