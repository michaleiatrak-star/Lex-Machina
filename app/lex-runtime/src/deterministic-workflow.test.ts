...
describe("deterministic legal workflow", () => {
  ...
  it(
    "allows a narrow case-law answer without forcing the full Faza 4 report",
    () => {
      const registry = fixture();
      const plan =
        createDeterministicWorkflowPlan(
      ...
      const report =
        evaluateDeterministicWorkflowOutput(
          plan,
          "Zweryfikowana odpowiedź dotycząca jednej sygnatury, bez deklarowania pełnego raportu."
        );
  ...
  it(
    "requires the case-law Faza 4 skeleton when a full report is claimed",
    () => {
      const registry = fixture();
      const plan =
        createDeterministicWorkflowPlan(
      ...
      const complete = [
        "RAPORT ORZECZEŃ: test",
        "WSKAŹNIK POKRYCIA PRZESŁANEK:",
        "PLAN MINIMUM (Zasada 11)",
        "[A] ORZECZENIA WSPIERAJĄCE TEZĘ",
        "[B] LINIA PRZECIWNA — nie odnaleziono po wyczerpującym wyszukiwaniu"
      ].join("\n");
