      ...
      pages: [
        {
          page: 1,
          text:
            "Jan Kowalski, PESEL 44051401458, opis sprawy " +
            "A".repeat(900)
        },
        {
          page: 2,
...
const ocr: OcrEngine = {
  async recognizePages(_data, pages) {
    return pages.map((page) => ({
      page,
      text:
        "Zeskanowana strona: Anna Nowak, e-mail anna@example.pl.",
      confidence: 0.98,
      lineCount: 1,
...
for (const secret of [
  ...
  if (publicText.includes(secret)) {
    throw new Error(
      `G29 public document chunks exposed protected data: ${secret}`
    );
  }
}
