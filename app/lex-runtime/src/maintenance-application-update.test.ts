...
describe(
  ...
  () => {
    it(
      "persists an explicit unsigned receipt when the temporary policy accepts SHA-256 + ProductVersion",
      async () => {
        const bytes =
          Buffer.from(
        ...
        const verify =
          vi.fn(
            (
              ...
              return {
                verification:
                  "UNSIGNED_ALLOWED" as const,
                subject: null,
                thumbprint: null,
                productVersion:
                  "0.1.4",
                warning:
                  "TEMPORARY_UNSIGNED_UPDATE_ALLOWED" as const
              };
            }
          );
