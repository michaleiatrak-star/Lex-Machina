...
export type Restoration = {
  ...
  caseMissing?: boolean;
  // A verb or role word next to the token disagrees with the key's gender or
  // number ("[osoba żeńska] wniósł", "[rodzina] wniosła"): checked by a person.
  agreement?: string;
};

type Agreement = "sg-m" | "sg-f" | "pl-m" | "pl-f";
...
const ROLE_AGREEMENT: Record<string, Agreement> = {
...
function expectedAgreement(entity: ReturnType<PseudonymizationVault["entity"]>): Agreement | null {
  if (!entity) return null;
  // A firm agrees through "spółka"/"firma": feminine.
  if (entity.type === "organization") return "sg-f";
  if (entity.status === "gender_ambiguous" || entity.warnings.includes("GENDER_HEURISTIC")) return null;
  if (entity.gender !== "m1" && entity.gender !== "f") return null;
  return `${entity.number === "pl" ? "pl" : "sg"}-${entity.gender === "f" ? "f" : "m"}` as Agreement;
}

/** Past-tense form after a subject: wniósł / wniosła / wnieśli / wniosły (zrobił, poszedł, mógł, wziął). */
function verbAgreement(word: string): Agreement | null {
  if (word.length < 4 || word !== word.toLocaleLowerCase("pl")) return null;
  if (/(?:[aeiouyęąó]|[śzdrkgn])li$/u.test(word)) return "pl-m";
  if (/(?:[aeiouyęąó]|[szdrkg])ły$/u.test(word)) return "pl-f";
  if (/(?:[aeiouyęąó]|[szdrkg])ła$/u.test(word)) return "sg-f";
  if (/(?:[aeiouyęąó]|[szdrkg])ł$/u.test(word)) return "sg-m";
  return null;
}
 ...
 */
export function agreementIssues(
  text: string,
  entityOf: (baseToken: string) => ReturnType<PseudonymizationVault["entity"]>
): Map<number, string> {
  const issues = new Map<number, string>();
  const pattern = /\[((?:LMPII:D\d{2}|PII):PERSON:\d{4})(?:\|([A-Z]{2,4}))?\]/g;
  for (const match of text.matchAll(pattern)) {
    // Only a subject: nominative or no case written.
    if (match[2] && match[2] !== "NOM") continue;
    ...
    if (/^\s*(?:,|i|oraz)\s*\[(?:LMPII|PII):/u.test(after)) continue;
    const before = text.slice(Math.max(0, match.index! - 40), match.index!);
    if (/\[(?:LMPII|PII):[^\]]+\]\s*(?:,|i|oraz)\s*$/u.test(before)) continue;
