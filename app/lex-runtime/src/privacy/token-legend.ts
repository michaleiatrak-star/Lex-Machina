...
/**
 * What a model may know about a placeholder: its kind and, for a person,
 * grammatical gender; whether it stands for several persons (a family named
 * together) or for a firm named after a person. Never the name itself.
 */
export type PlaceholderGrammar = {
  token: string;
  kind: "PERSON" | "ADDRESS";
  gender?: TokenGender;
  // "group": several persons (Kowalscy); "organization": a firm's name.
  entity?: "person" | "group" | "organization";
  legalForm?: string;
  // Organization: the person whose name the firm carries, when also in the text.
  owner?: string;
};
...
export function genderOf(
  ...
  if (!entity || entity.status === "gender_ambiguous" || entity.warnings.includes("GENDER_HEURISTIC")) return "unknown";
...
export function placeholderGrammar(
  ...
  for (const entry of seen.values()) {
    if (entry.entity !== "organization") continue;
    ...
    if (owner) entry.owner = owner.token;
  }
  return [...seen.values()];
}
...
const GROUP_LINE: Record<TokenGender, string> = {
...
function describe(entry: PlaceholderGrammar): string {
  if (entry.kind === "ADDRESS") return "adres";
  ...
  if (entry.entity === "group") return GROUP_LINE[entry.gender ?? "unknown"];
...
const PARTY_LIST = new RegExp(
  String.raw`(?<![\p{L}])(\p{L}{4,})\s+(${PERSON_TOKEN}(?:\s*(?:,|\bi\b|\boraz\b)\s*${PERSON_TOKEN})+)`,
  "gu"
);
 ...
 */
export function partyGroups(texts: string[], isRole: (word: string) => boolean): PartyGroup[] {
  ...
  for (const text of texts) {
    for (const match of text.matchAll(PARTY_LIST)) {
      ...
      if (!groups.has(key)) groups.set(key, { role, tokens });
    }
  }
  return [...groups.values()];
}

function partyLine(group: PartyGroup, byToken: Map<string, PlaceholderGrammar>): string {
...
export function placeholderKeyPrompt(
  entries: PlaceholderGrammar[],
  parties: PartyGroup[] = []
): string | null {
