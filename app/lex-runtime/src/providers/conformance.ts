...
export type ProviderConformanceCase = {
  id: string;
  pass: boolean;
  detail?: string;
};
...
export async function runProviderConformance(
  ...
  const cases: ProviderConformanceCase[] = [];
  ...
  cases.push({
    ...
    pass:
      basic.fullText === contentChunks.join("") &&
  ...
  const toolParams: ProviderStreamParams = {
    model: "conformance-model",
    systemPrompt: "System",
  ...
  cases.push({
    ...
    pass:
      !adapter.capabilities.modelDiscovery ||
      models.length > 0
  });
