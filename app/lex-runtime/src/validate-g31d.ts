...
const ast = {
  ...
  styleProfile: "lex-classic-clean-v1" as const,
  blocks: [{
    type: "paragraph" as const,
    content: [
      { type: "text" as const, text: "Klient: " },
      { type: "pii_ref" as const, alias: "[LMPII:D01:PERSON:0001]" }
    ]
  }]
};
...
let final: Awaited<ReturnType<typeof renderer.deanonymize>> | undefined;
try {
  const deterministic = first.data.equals(second.data);
  const tokenized = await renderer.validate("docx", first.data);
  final = await renderer.deanonymize(
    "docx",
    first.data,
    new Map([["[LMPII:D01:PERSON:0001]", "Jan Kowalski"]])
  );
