...
async function satisfyCaseLawWorkflowPreflight(
  ...
  if (!readTool || !params.runTools) {
    throw new Error(
      "G23_CASE_LAW_PREFLIGHT_TOOL_MISSING"
    );
  }
...
implements ProviderAdapter {
  ...
  readonly label =
    "G23 deterministic quote provider";
...
function body() {
  return {
    query:
      "Zweryfikuj dokładny cytat z orzeczenia SN III CZP 25/11.",
    provider: "openai",
    model: "g23-model",
    primarySkill: DR02,
    mode: "PRAWNIK"
...
process.stdout.write(
  JSON.stringify({
    gate:
      "G23_EXACT_SN_CASE_QUOTE_EVIDENCE",
    result:
      pass
        ? "PASS"
        : "BLOCKED",
