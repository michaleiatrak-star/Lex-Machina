import type { EditableModel, EditableRun } from "./office-edit.js";

export const MAX_FIRM_TEMPLATES = 8;
const MAX_TEMPLATE_CHARS = 60_000;
const CHUNK_CHARS = 6_000;

function runsText(runs: EditableRun[]): string {
 ...
 */
export function templateText(model: EditableModel): string {
  const lines: string[] = [];
  if (model.kind === "sheet") {
...
export function templateChunks(text: string): Array<{ index: number; pageStart: number; pageEnd: number; text: string }> {
  const chunks: string[] = [];
  let current = "";
