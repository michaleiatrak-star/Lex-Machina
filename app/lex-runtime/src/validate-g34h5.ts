...
try {
  ...
  const clear = Buffer.from(
    "%PDF-client-pesel-90010112345-legacy"
  );

  const localCase =
    await legacy.createCase({
      createdByUserId: ownerId,
      keyVersion: 1,
      displayName: "Legacy H5"
    });
  const uploaded =
    await legacy.saveUpload({
  ...
  const encryptedRoundTrip =
    payload.equals(clear);
  payload.fill(0);
  ...
  const plaintextRemaining =
    await containsNeedle(
      caseRoot,
      Buffer.from(
        "client-pesel-90010112345-legacy"
      )
    );
  ...
  const pass =
    first.remainingLegacyPlaintext ===
      false &&
    first.migratedUploads.includes(
      uploaded.uploadId
    ) &&
    secureMatch?.storage ===
      "ENCRYPTED_LME1" &&
    ...
    blocked.remainingLegacyPlaintext ===
      true &&
    blocked.blockedEntries.includes(
      "artifacts:NONEMPTY_LEGACY_DIRECTORY"
    ) &&
    blockerPreserved;
  ...
  process.stdout.write(
    JSON.stringify({
      gate:
        "G34H5_LEGACY_PLAINTEXT_MIGRATION",
      result:
        pass ? "PASS" : "BLOCKED",
      verifiedBeforePlaintextRemoval:
        encryptedRoundTrip,
      plaintextRemaining,
      sameUploadIdPreserved:
        secureMatch?.uploadId ===
        uploaded.uploadId,
      idempotent,
      unknownLegacyBlocked:
        blocked.remainingLegacyPlaintext,
      unknownLegacyPreserved:
        blockerPreserved,
