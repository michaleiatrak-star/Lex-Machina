...
const support =
  read(
    "app/lex-runtime/src/support-service.ts"
  );
const http =
  read(
...
const test =
  read(
    "app/lex-runtime/tests/g37d-support-entitlement.test.ts"
  );

const checks = {
  noUniversalSupportPassword:
    !support.includes(
      "servicePassword"
    ) &&
    !trust.includes(
  ...
  nativeHeaderCannotBeSpoofed:
    trust.includes(
      'lower != "x-lex-service-authorization"'
    ),
  noCaseDataSupportRoutes:
    http.includes(
  ...
  deterministicRegression:
    test.includes(
      "signed SERVICE support entitlement"
    ) &&
    test.includes(
...
console.log(
  JSON.stringify(
    {
      gate:
        "G37D_SIGNED_SERVICE_SUPPORT_ENTITLEMENT",
      result:
        pass
          ? "PASS"
          : "BLOCKED",
