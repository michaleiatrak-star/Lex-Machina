export type DomainLockReport = {
  ...
  result: "PASS" | "BLOCKED";
  criminalDomainLoaded: boolean;
  criminalLegalReferences: string[];
  unsupportedCriminalSignals: string[];
};

export type RateCompletenessReport = {
  gate:
    "G39I_RATE_COMPLETENESS";
  result:
    "PASS" | "BLOCKED";
  triggered: boolean;
  numericRateLines: number;
  unverifiedNumericRateLines: number;
  aggregateClaim: boolean;
  hasExplicitInterval: boolean;
  hasSeriesTable: boolean;
  missing: string[];
};
...
export function evaluateDomainLock(args: {
  ...
  const criminalDomainLoaded =
    args.loadedSkills.some(
      (skill) =>
        skill.startsWith(
          "dr-03-"
        )
  ...
  const unsupportedCriminalSignals =
    unique(
      [
        ...args.text.matchAll(
          HIGH_RISK_CRIMINAL_SIGNAL
        )
      ].map(
        (match) =>
          match[0] ?? ""
...
const SERIES_HEADER =
  /\|\s*Od\s*\|\s*Do\s*\|\s*Stawka\s*\|/iu;

export function evaluateRateCompleteness(
  text: string
): RateCompletenessReport {
  const triggered =
    RATE_TOPIC.test(text);
  if (!triggered) {
    return {
      gate:
        "G39I_RATE_COMPLETENESS",
  ...
  const numericLines =
    lines.filter(
      (line) =>
        RATE_TOPIC.test(line) &&
        NUMERIC_RATE.test(line)
    );
  const unverified =
    numericLines.filter(
      (line) =>
        !VERIFIED_OR_GAP.test(
          line
        )
    );
  ...
  const hasExplicitInterval =
    EXPLICIT_INTERVAL.test(
      text
    );
  const hasSeriesTable =
    SERIES_HEADER.test(text);
  const missing:
    string[] = [];
