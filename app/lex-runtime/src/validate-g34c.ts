...
try {
  ...
  const owner =
    await auth.bootstrap({
      ...
      displayName: "Owner",
      password:
        "G34C owner bezpieczne haslo 2026"
    });
  const ownerContext = {
    user: owner.user,
  ...
  const target =
    await auth.createUser(
      ownerContext,
      {
        ...
        displayName: "Analyst",
        password:
          "G34C analyst bezpieczne haslo 2026"
      }
    );
