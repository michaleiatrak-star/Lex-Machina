...
try {
  ...
  const owner =
    await auth.bootstrap({
      ...
      displayName:
        "G34H3 Owner",
      password:
        "G34H3 walidacyjne bezpieczne haslo 2026"
    });
  const actor = {
    user: owner.user,
  ...
  const sourceText =
    "Jan Kowalski jest powodem. PESEL 44051401458. Treść sprawy pozostaje lokalna.";
  const ingestor =
  ...
  process.stdout.write(
    JSON.stringify({
      gate:
        "G34H3_ENCRYPTED_DOCUMENT_PERSISTENCE",
      result:
        pass
          ? "PASS"
          : "BLOCKED",
      sourceAndProtectedLmePresent:
        fs.existsSync(
          path.join(
            secureDocumentRoot,
            "source.lme"
