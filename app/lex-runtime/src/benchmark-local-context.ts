...
function parseProfiles(
  ...
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_PROFILES_INVALID"
    );
  }
...
async function tokenize(
  ...
  if (!response.ok) {
    fail(
      `LOCAL_CONTEXT_BENCHMARK_TOKENIZE_HTTP_${response.status}`
    );
  }
  const payload =
  ...
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_TOKENIZE_INVALID"
    );
  }
...
async function fitPrompt(args: {
  ...
  for (
    ...
    if (
      ...
      fail(
        "LOCAL_CONTEXT_BENCHMARK_EMPTY_TOKENIZATION"
      );
    }
...
async function recall(
  ...
  if (!response.ok) {
    fail(
      `LOCAL_CONTEXT_BENCHMARK_CHAT_HTTP_${response.status}`
    );
  }
  const payload =
    await response.json() as {
  ...
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_CHAT_INVALID"
    );
  }
  return output.trim();
}
...
async function main(): Promise<void> {
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_WINDOWS_REQUIRED"
    );
  }
  ...
  if (!outputArg) {
    fail(
      "LOCAL_CONTEXT_BENCHMARK_OUTPUT_REQUIRED"
    );
  }
  const outputPath =
  ...
  if (!identity) {
    fail(
      "LOCAL_CONTEXT_BENCHMARK_MODEL_NOT_INSTALLED"
    );
  }
  ...
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_MODEL_MISMATCH"
    );
  }
  const modelId =
    identity.modelId;
  ...
  if (!descriptor) {
    fail(
      "LOCAL_CONTEXT_BENCHMARK_MODEL_DESCRIPTOR_MISSING"
    );
  }
  ...
  if (
    ...
    fail(
      "LOCAL_CONTEXT_BENCHMARK_PROFILE_OUTSIDE_MODEL_POLICY"
    );
  }
    ...
    for (
      ...
      try {
        ...
        if (
          ...
          fail(
            "LOCAL_CONTEXT_BENCHMARK_QUALIFICATION_MISSING"
          );
        }
        ...
        if (
          ...
          fail(
            "LOCAL_CONTEXT_BENCHMARK_TOKENIZER_CALIBRATION_REQUIRED"
          );
        }
        ...
        for (
          ...
          const passkey =
            `LEXCTX-${contextTokens}-P${index + 1}-A7F3C9`;
          const fitted =
            await fitPrompt({
              endpoint,
              targetTokens,
  ...
  const report = {
    schemaVersion: 1,
    kind:
      "LEX_MACHINA_LOCAL_CONTEXT_BENCHMARK",
    modelId,
    modelSha256:
      identity.sha256,
    modelPack: {
