 ...
 */
export function genericWords(file = process.env.LEX_GENERIC_WORDS ?? defaultPath()) {
    if (cached)
        return cached;
    try {
        ...
        process.stderr.write(`LEX_GENERIC_WORDS_UNAVAILABLE:${error instanceof Error ? error.message : String(error)}\n`);
 ...
 */
export function personPart(text, span) {
    ...
    if (previous && words.personOnlyTriggers.has(previous))
        return span;
    if (words.institutions.has(first))
        return null;
    let index = 0;
    while (index < tokens.length && words.partyRoles.has(tokens[index][0].toLowerCase()))
        index += 1;
    if (index === 0)
        return span;
...
export function isAmbiguousPerson(span) {
    if (span.ambiguous)
        return true;
...
export function sentenceAround(text, span) {
    ...
    return (text.slice(start, span.start) + "⟦" + text.slice(span.start, span.end) + "⟧" + text.slice(span.end, end))
        .replace(/\s+/g, " ")
        .trim();
}
