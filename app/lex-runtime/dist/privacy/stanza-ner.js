...
function defaultWorkerPath() {
    ...
    return path.resolve(here, "../../../privacy/stanza_ner_worker.py");
}
export class LocalStanzaNamedEntityRecognizer {
    ...
    async recognize(text) {
        if (!text.trim())
            return [];
        const tempRoot = await mkdtemp(path.join(os.tmpdir(), "lex-stanza-ner-"));
            ...
            await new Promise((resolve, reject) => {
                ...
                child.once("exit", (code) => {
                    ...
                    if (code === 0)
                        ...
                        reject(new Error(`Local Stanza NER worker failed with exit code ${code}: ${stderr.trim()}`));
                    }
                });
            });
