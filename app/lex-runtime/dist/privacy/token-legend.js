...
export function genderOf(vault, token) {
    ...
    if (!entity || entity.status === "gender_ambiguous")
        return "unknown";
 ...
 */
export function placeholderKeyPrompt(entries) {
    if (!entries.length)
        return null;
    const lines = entries
        .slice(0, 200)
        .map((entry) => `- ${entry.token}: ${entry.kind === "PERSON" ? GENDER_LINE[entry.gender ?? "unknown"] : "adres"}`);
    return [
