...
const ACTS = [
    ...
    },
    {
        ...
        eli: "DU/2026/468",
        baseEli: "DU/1964/296",
        sourceUrl: "https://api.sejm.gov.pl/eli/acts/DU/2026/468/text.html",
        sourceKind: "consolidated_text",
        registryAsOf: REGISTRY_AS_OF,
        aliases: [
            "KPC",
    ...
    },
    {
        ...
        eli: "DU/2026/490",
        baseEli: "DU/1997/555",
        sourceUrl: "https://api.sejm.gov.pl/eli/acts/DU/2026/490/text.html",
        sourceKind: "consolidated_text",
        registryAsOf: REGISTRY_AS_OF,
        aliases: [
            "KPK",
...
for (const entry of ACTS) {
    ...
    for (const key of aliases) {
        ...
        if (existing && existing.id !== entry.id) {
            throw new Error("Legal act alias collision in runtime registry: " + key);
        }
...
export class DeterministicLegalActResolver {
    resolve(value) {
        ...
        if (!normalized) {
            throw new LegalActResolutionError("Legal act identifier cannot be empty.", "EMPTY_LEGAL_ACT");
        }
        const entry = BY_ALIAS.get(normalized);
        if (!entry) {
            throw new LegalActResolutionError("Legal act is not present in the runtime registry.", "UNKNOWN_LEGAL_ACT");
        }
        return {
            id: entry.id,
            title: entry.title,
