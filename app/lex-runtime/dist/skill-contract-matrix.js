...
function addSkillMetadataChecks(registry, name, checks) {
    ...
    checks.push({
        ...
        detail: skill.body.trim().length > 0
            ? `${skill.body.length} chars`
            : "empty SKILL.md body"
    });
}
export function validateSkillContractMatrix(registry) {
    const checks = [];
    const discoveredDr = [...registry.skills.keys()]
        .filter((name) => /^dr-\d{2}-/.test(name))
        .sort();
    const expectedDr = [...EXPECTED_DR_SKILLS].sort();
    const missingDrSkills = expectedDr.filter((name) => !registry.skills.has(name));
    const unexpectedDrSkills = discoveredDr.filter((name) => !EXPECTED_DR_SKILLS.includes(name));
    const missingExecutionSkills = CORE_EXECUTION_SKILLS.filter((name) => !registry.skills.has(name));
    checks.push({
        id: "exact-dr-count",
        target: "DR",
        pass: discoveredDr.length === EXPECTED_DR_SKILLS.length &&
    ...
    const routingMap = registry.resolveResource("prawo-polskie-v2", "prawo-polskie-v2/ROUTING-MAP.md");
