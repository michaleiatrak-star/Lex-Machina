...
async function buildExecutionAudit(id) {
...
if (issues.length > 0) {
    ...
    const legacyToolPresence = Object.fromEntries(legacyTools.map((name) => [
        name,
        fs.existsSync(path.join(lexRoot, "shared", "tools", name))
    ]));
    const pass = positive.result === "PASS" &&
    ...
    process.stdout.write(JSON.stringify({
        ...
        note: "README documents legacy Python export tools, but the current corpus contains only shared/tools/README.md. Runtime-native G10 preserves the documented neutral verification-log contract."
    }, null, 2) + "\n");
    if (!pass)
        process.exitCode = 1;
}
