...
export class LocalCaseFileStore {
    ...
    async createCase(input) {
        const requestedCaseId = typeof input === "object" &&
            input
            ? input.caseId
            : undefined;
        ...
        const displayName = typeof input === "string"
            ? input
            : input?.displayName;
        const createdByUserId = typeof input === "object" &&
            input
            ? input.createdByUserId
            : undefined;
        const keyVersion = typeof input === "object" &&
            input
            ? input.keyVersion
            : undefined;
        const caseKind = typeof input === "object" &&
            input
            ? input.caseKind ??
                "MATTER"
            : "MATTER";
        if (![
            ...
            throw new Error("INVALID_CASE_KIND");
        }
        if (createdByUserId !== undefined &&
            !/^user_[a-f0-9]{32}$/
                .test(createdByUserId)) {
            throw new Error("INVALID_CASE_OWNER_ID");
        }
        if (keyVersion !== undefined &&
            (!Number.isInteger(keyVersion) ||
                keyVersion < 1)) {
            throw new Error("INVALID_CASE_KEY_VERSION");
        }
        ...
        await writeFile(path.join(dir, "case.json"), JSON.stringify(metadata, null, 2), {
            encoding: "utf8",
            flag: "wx"
        });
        return metadata;
    }
    async readCaseMetadata(caseId) {
        const raw = JSON.parse(await readFile(path.join(this.caseDir(caseId), "case.json"), "utf8"));
        if (raw.caseId !== caseId ||
            typeof raw.createdAt !==
                "string") {
            throw new Error("CASE_METADATA_INVALID");
        }
        return {
            caseId,
            createdAt: raw.createdAt,
            caseKind: raw.caseKind ===
                "FIRM_KNOWLEDGE"
                ? "FIRM_KNOWLEDGE"
                : "MATTER",
    ...
    async listLegacyCases() {
        const base = path.join(this.rootDir, "cases");
        let entries;
        try {
    ...
    async updateCaseSecurityMetadata(caseId, args) {
        if (!/^user_[a-f0-9]{32}$/
            .test(args.createdByUserId) ||
            !Number.isInteger(args.keyVersion) ||
            args.keyVersion < 1) {
            throw new Error("INVALID_CASE_SECURITY_METADATA");
        }
        const metadata = await this.readCaseMetadata(caseId);
        if (metadata.createdByUserId ||
            metadata.keyVersion) {
            throw new Error("CASE_ALREADY_SECURITY_BOUND");
        }
        const target = path.join(this.caseDir(caseId), "case.json");
    ...
    async updateCaseKeyVersion(caseId, keyVersion) {
        ...
        const metadata = await this.readCaseMetadata(caseId);
        if (!metadata.createdByUserId) {
            throw new Error("LEGACY_CASE_REQUIRES_IMPORT");
        }
        const target = path.join(this.caseDir(caseId), "case.json");
        const temp = target + ".partial";
        await writeFile(temp, JSON.stringify({
    ...
    async updateCaseLifecycleMetadata(caseId, args) {
        const metadata = await this.readCaseMetadata(caseId);
        const next = {
            ...metadata,
        ...
        if (Object.prototype
            .hasOwnProperty.call(args, "displayName")) {
            const cleaned = args.displayName
                ?.normalize("NFKC")
                .trim()
                .slice(0, 160) ??
                "";
        ...
        if (Object.prototype
            .hasOwnProperty.call(args, "archivedAt")) {
            if (args.archivedAt) {
                next.archivedAt =
                    args.archivedAt;
            }
    ...
    async removeCase(caseId) {
        await rm(this.caseDir(caseId), {
            recursive: true,
            force: true
        });
    }
    async assertCase(caseId) {
        await access(path.join(this.caseDir(caseId), "case.json"));
    }
    async listUploads(caseId) {
        await this.assertCase(caseId);
        const incoming = path.join(this.caseDir(caseId), "incoming");
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            try {
                const parsed = JSON.parse(await readFile(path.join(incoming, entry.name, "manifest.json"), "utf8"));
        ...
        return uploads.sort((a, b) => b.storedAt.localeCompare(a.storedAt));
    }
    async saveUpload(args) {
        await this.assertCase(args.caseId);
        const uploadId = "upload_" +
            randomBytes(16).toString("hex");
        const uploadDir = path.join(this.caseDir(args.caseId), "incoming", uploadId);
        const originalDir = path.join(uploadDir, "original");
        await mkdir(originalDir, {
            recursive: true
        });
        const filename = safeFilename(args.filename);
        const finalPath = path.join(originalDir, filename);
        const tempPath = finalPath + ".partial";
        await writeFile(tempPath, args.data, { flag: "wx" });
        ...
        try {
            ...
            if (isZip &&
                ...
                extracted =
                    await this.extractZip(finalPath, uploadDir);
            }
            const manifest = {
                caseId: args.caseId,
                uploadId,
    ...
    async extractZip(inputPath, uploadDir) {
        const outputDir = path.join(uploadDir, "extracted");
        const workerManifest = path.join(uploadDir, "extracted-manifest.json");
        try {
            ...
            const parsed = JSON.parse(await readFile(workerManifest, "utf8"));
            ...
            await rm(workerManifest, { force: true });
            throw error;
        }
    }
}
