...
const checks = {
    postInstallSelfTest: config.bundle?.windows?.nsis?.installerHooks === "./windows/hooks.nsh" &&
        hooks.includes("lex-runtime-sidecar.exe") &&
        hooks.includes("--self-test") &&
        hooks.includes("Abort"),
    firstAdminSeeded: server.includes('loginName: "admin"') &&
        server.includes('password: "admin"') &&
        server.includes("passwordSetupPending: true"),
    bootstrapSessionNotExposed: server.includes("logoutAuthorization(") &&
        server.includes("bootstrap.sessionToken"),
    manualFirstLogin: authUi.includes("temporaryAdminCredentialsActive") &&
        authUi.includes('? "admin"') &&
        authUi.includes("<strong>Pierwsze logowanie</strong>") &&
        authUi.includes("Login <b>admin</b>, hasło <b>admin</b>."),
    warnedPasswordReplacement: authUi.includes("passwordSetupPending === true") &&
    ...
    firstRunIsWarningNotBlockade: !authUi.includes("Zanim przejdziesz dalej") &&
        !/passwordSetupPending === true\s*\)\s*\{\s*return \(/.test(authUi) &&
        authUi.includes("<App"),
    nativeDeanonymizationReauth: authoring.includes("isDesktopShell()") &&
        trust.includes("__LEX_NATIVE_REAUTH__"),
    providerCredentialPersistence: trust.includes("PROVIDER_KEYRING_SERVICE") &&
        trust.includes("restore_provider_credentials") &&
        trust.includes("persist_provider_credential"),
    providerOrOptionalLocalAiPolicy: read("app/installer/generate-component-lock.ps1")
        .includes('expectedUserActionAfterInstall = "PROVIDER_API_KEY_OR_OPTIONAL_LOCAL_AI_SETUP"') &&
