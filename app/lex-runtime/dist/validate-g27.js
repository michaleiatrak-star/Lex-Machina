        ...
        return {
            ...
            pages: Array.from({ length: 80 }, (_, index) => {
                ...
                return {
                    page,
                    text: `Strona ${page}. ` +
                        "Treść dokumentu prawnego z polskimi znakami: ą ć ę ł ń ó ś ź ż. ".repeat(page % 7 === 0 ? 90 : 4)
                };
            })
        };
    }
...
const ocr = {
    async recognizePages(_data, pages) {
        return pages.map((page) => ({
            page,
            text: `OCR strony ${page}: zeskanowana treść została odczytana lokalnie w języku polskim.`,
            confidence: 0.96,
            lineCount: 1,
...
const ingestor = new CompleteDocumentIngestor(source, ocr, {
    maxBytes: 1024 * 1024,
    maxPages: 1000,
...
const result = await ingestor.ingest(new TextEncoder().encode("G27 deterministic document fixture"));
if (!result.complete || result.totalPages !== 80) {
    throw new Error("G27 did not account for all document pages.");
}
if (result.ocrPages !== 7) {
    throw new Error(`G27 expected 7 OCR pages, got ${result.ocrPages}.`);
}
if (result.chunks.reduce((sum, chunk) => sum + chunk.sourceChars, 0) !== result.sourceChars) {
