...
const descriptor = new DeterministicLegalActResolver().resolve("KC");
const checker = new TemporalSourceFreshnessChecker();
const attempts = [];
let result;
for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (result.status !==
        ...
        !result.reason?.endsWith("_FETCH_FAILED") ||
        attempt === 3) {
        break;
    }
...
if (!result) {
    throw new Error("G19_LIVE_RESULT_MISSING");
}
const registryStillCurrent = result.currentEli === descriptor.eli;
...
const upstreamFetchUnavailable = result.status ===
    "SOURCE_METADATA_UNAVAILABLE" &&
    Boolean(result.reason?.endsWith("_FETCH_FAILED"));
const productionVerificationPermitted = result.status === "CURRENT";
const failClosedDuringUpstreamOutage = upstreamFetchUnavailable &&
    !productionVerificationPermitted;
const pass = liveStateConfirmed ||
    failClosedDuringUpstreamOutage;
process.stdout.write(JSON.stringify({
    gate: "G19_LIVE_TEMPORAL_FRESHNESS_PROBE",
    result: pass ? "PASS" : "BLOCKED",
    probeMode: liveStateConfirmed
    ...
    currentPromulgation: result.currentPromulgation ?? null,
    amendmentsAfter: result.amendmentsAfter.map((amendment) => ({
        eli: amendment.eli,
    ...
    registryStillCurrent,
    repealedConsolidatedText: result.status ===
        "REPEALED_CONSOLIDATED_TEXT",
    productionVerificationPermitted,
    attempts,
    note: liveStateConfirmed
