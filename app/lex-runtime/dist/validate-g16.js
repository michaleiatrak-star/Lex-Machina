...
function g16FreshnessFetcher(input) {
    ...
    return Promise.reject(new Error("G16_UNEXPECTED_FRESHNESS_URL:" +
        url));
}
class VerificationProvider {
    mode;
    id = "openai";
    label = "G16 deterministic verification provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        const readTool = params.tools?.find((tool) => tool.function.name ===
            "read_legal_resource");
        const verificationTool = params.tools?.find((tool) => tool.function.name ===
            "verify_legal_reference");
        if (!readTool ||
            !verificationTool ||
            !params.runTools) {
            throw new Error("G16_REQUIRED_TOOL_MISSING");
        }
        ...
        if (readResults.length !==
            ...
            readResults.some((result) => typeof result.content !==
                "string" ||
                !result.content.trim())) {
            throw new Error("G16_STATUTE_PREFLIGHT_READ_FAILED");
        }
        if (this.mode === "fake-marker") {
            return {
                fullText: "Znaczenie ma art. 5 KC. ✅ [VER: https://eli.gov.pl/fake, 2026-09-15]"
            };
        }
        const toolResults = await params.runTools([{
                id: `g16-${this.mode}-verify-1`,
        ...
        const raw = toolResults[0]?.content ??
            "{}";
        const toolPayload = JSON.parse(raw);
        const marker = typeof toolPayload.marker ===
            "string"
            ? toolPayload.marker
            : "⚠️ [NIEWERYFIKOWANE]";
        return {
...
function executor(registry, mode) {
    const providers = new ProviderRegistry();
    providers.register(new VerificationProvider(mode));
    const verifier = new OfficialLegalSourceVerifier(async () => new Response(mode === "verified"
...
function requestBody() {
    return {
        query: "Zweryfikuj powołanie art. 5 KC i użyj wyłącznie świeżego źródła urzędowego.",
        provider: "openai",
        model: "g16-model",
        primarySkill: DR02,
        mode: "PRAWNIK"
...
if (issues.length > 0) {
    ...
    const verifiedHttp = await request(appFor(registry, "verified"))
        ...
        .send(requestBody());
    const fakeMarkerHttp = await request(appFor(registry, "fake-marker"))
        .post("/api/sessions/execute")
        .send(requestBody());
    const unverifiedHttp = await request(appFor(registry, "unverified"))
        .post("/api/sessions/execute")
        .send(requestBody());
    const verified = verifiedHttp.body;
    const fake = fakeMarkerHttp.body;
    const unverified = unverifiedHttp.body;
    const verifiedAudit = verified.audit &&
        typeof verified.audit === "object"
        ? verified.audit
        : {};
    const verifiedSummary = verified.verification &&
        typeof verified.verification === "object"
        ? verified.verification
        : {};
    const fakeSummary = fake.verification &&
        typeof fake.verification === "object"
        ? fake.verification
        : {};
    const unverifiedSummary = unverified.verification &&
        typeof unverified.verification === "object"
        ? unverified.verification
        : {};
    const pass = verifiedHttp.status === 200 &&
