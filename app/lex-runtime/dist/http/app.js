...
export function createLexHttpApp(options) {
    ...
    app.get("/api/model-routing/preferences", (_req, res) => {
        if (!options.authService) {
            res.status(503).json({
                error: "AUTH_SERVICE_UNAVAILABLE"
            });
    ...
    app.put("/api/model-routing/preferences", (req, res) => {
        if (!options.authService) {
            res.status(503).json({
                error: "AUTH_SERVICE_UNAVAILABLE"
            });
        ...
        const body = req.body &&
            ...
            ? req.body
            : null;
        const provider = typeof body
            ?.auxiliaryProvider ===
            "string"
            ? body
                .auxiliaryProvider
            : "";
        const model = typeof body
            ?.auxiliaryModel ===
            "string"
            ? body
                .auxiliaryModel
                .trim()
            : "";
        const enabled = body?.auxiliaryEnabled;
        if (typeof enabled !==
            "boolean" ||
            !isProviderId(provider) ||
            model.length < 1 ||
            model.length > 256) {
            res.status(400).json({
                error: "INVALID_MODEL_ROUTING_PREFERENCES"
            });
            return;
        }
        const actor = responseAuthContext(res);
        try {
            res.json(options.authService
                .setModelRoutingPreferences(actor, {
                auxiliaryEnabled: enabled,
    ...
    app.post("/api/sessions/execute", async (req, res) => {
        ...
        if (options.authService) {
            const actor = responseAuthContext(res);
            const preferences = options.authService
                .getModelRoutingPreferences(actor);
