...
const checks = {
    realOnlineBootstrapJob: workflow.includes("G33D Windows online installer build and acceptance") &&
        workflow.includes("build-windows-online.ps1") &&
        !workflow.includes("NSISBI"),
    realNsisBundle: workflow.includes("tauri build --bundles nsis") &&
        workflow.includes("windows-installer-acceptance.ps1"),
    artifactPublished: workflow.includes("actions/upload-artifact") &&
        workflow.includes("LexMachina-Windows-Online-Installer"),
    installedCopyBootstrappedAndTested: acceptance.includes("/S") &&
    ...
    monolithicBundleRejected: acceptance.includes("MONOLITHIC_BUNDLE_TOO_LARGE"),
    systemNodePythonExcluded: acceptance.includes("$env:PATH = \"$env:SystemRoot\\System32;$env:SystemRoot\"") &&
        acceptance.includes("privateNode") &&
        acceptance.includes("privatePython"),
    firstRunWithoutProviderKey: acceptance.includes("first desktop startup without provider key") &&
        acceptance.includes("INSTALLER_ACCEPTANCE_DESKTOP_EARLY_EXIT"),
    actualOcrNerRendererSelftest: selftest.includes("SELFTEST_OCR_INFERENCE_FAILED") &&
        selftest.includes("SELFTEST_NER_INFERENCE_FAILED") &&
        selftest.includes("SELFTEST_DOCX_RENDER_FAILED"),
    missingOnlyAndHashChecked: bootstrap.includes("Test-CommandVersion") &&
    ...
    postInstallFailClosed: hooks.includes("windows-online-bootstrap.ps1") &&
        hooks.includes("--self-test") &&
        hooks.includes("Abort"),
    deterministicRuntimeResourceMapping: tauriConfig.bundle?.resources === undefined &&
        ...
        hooks.includes('RMDir /r "$INSTDIR\\runtime\\app"')
};
