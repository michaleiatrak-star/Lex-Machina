...
const text = "Jan Kowalski PESEL 44051401458 jest świadkiem, a Anna Nowak jest pełnomocnikiem.";
const janStart = text.indexOf("Jan Kowalski");
const peselStart = text.indexOf("44051401458");
const annaStart = text.indexOf("Anna Nowak");
const service = new LocalPolishPseudonymizer(new PseudonymizationVault());
...
if (result.text.includes("Jan Kowalski") ||
    !result.text.includes("44051401458") ||
    !result.text.includes("Anna Nowak") ||
    !result.text.includes("[PII:PERSON:0001]")) {
    throw new Error("G28A user privacy directives were not applied correctly.");
}
if (result.keptRanges.length !== 1 ||
    result.annotations[0]?.label !== "pełnomocnik" ||
    result.findings.filter((item) => item.source === "USER").length !== 1) {
    throw new Error("G28A user privacy metadata is incomplete.");
}
