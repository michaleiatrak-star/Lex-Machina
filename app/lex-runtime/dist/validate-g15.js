...
function appWithExecutor(executor) {
...
if (issues.length > 0) {
    ...
    safeProviders.register(new ScriptedProviderAdapter({ id: "openai" }));
    const safeExecutor = new SafeSessionExecutor(registry, new ProviderGateway(safeProviders));
    ...
    const unsafeExecutor = new SafeSessionExecutor(registry, new ProviderGateway(unsafeProviders));
    ...
    const safe = safeHttp.body;
    const unsafe = unsafeHttp.body;
    const safeAudit = typeof safe.audit === "object" && safe.audit !== null
        ? safe.audit
        : {};
    const unsafeAudit = typeof unsafe.audit === "object" && unsafe.audit !== null
        ? unsafe.audit
        : {};
    const unsafeReferences = Array.isArray(unsafe.blockedReferences)
        ? unsafe.blockedReferences
        : [];
    const pass = safeHttp.status === 200 &&
