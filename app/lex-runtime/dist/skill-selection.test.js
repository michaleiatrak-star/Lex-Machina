...
import { SKILL_SELECTION_ENVELOPE_PREFIX, parseSkillSelectionEnvelope, resolveAdditionalSkills } from "./skill-selection.js";
const roots = [];
afterEach(() => {
...
describe("skill selection", () => {
    it("strips the internal envelope and preserves manual skill choices", () => {
        const input = `${SKILL_SELECTION_ENVELOPE_PREFIX} {\"auto\":false,\"manual\":[\"terminy-procesowe\"]}\n` +
    ...
    it("carries a DR domain allow-list separately from manual skills", () => {
        const input = `${SKILL_SELECTION_ENVELOPE_PREFIX} ` +
            JSON.stringify({
                auto: true,
    ...
    it("parses execution and domain checkbox allow-lists independently", () => {
        ...
        expect(parseSkillSelectionEnvelope(input)).toEqual({
            ...
            executionRestrictionActive: true,
            modelRouted: false,
            workflowExecutionSkill: null
        });
    });
    it("lets an explicit empty execution allow-list disable all optional execution skills", () => {
        ...
        const selected = resolveAdditionalSkills(registry, "Przygotuj analizę umowy i raport dla klienta.", "dr-02-prawo-cywilne", true, [], [], false, [], true);
    ...
    it("limits automatic execution routing to checked skills without forcing their priority", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Przygotuj analizę umowy i raport dla klienta.", "dr-02-prawo-cywilne", true, [], [], false, [
            "analizator-umow-v1"
        ], true);
        expect(selected.executionSkills).toEqual([
            "analizator-umow-v1"
        ]);
        expect(selected.workflowExecutionSkill).toBe("analizator-umow-v1");
    });
    it("treats an explicit empty domain allow-list as primary-domain only", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Pracownik pozywa pracodawcę i pyta o terminy procesowe.", "dr-01-prawo-pracy", true, [], [], true);
    ...
    it("keeps every domain available when no allow-list is sent", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Pracownik pozywa pracodawcę; trzeba ocenić wypowiedzenie i przygotować pozew oraz terminy procesowe.", "dr-03-prawo-procesowe", true, []);
        expect(selected.domainSkills.length)
            .toBeGreaterThan(1);
    });
    it("restricts automatic domain routing to the allow-list", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Pracownik pozywa pracodawcę; trzeba ocenić wypowiedzenie i przygotować pozew oraz terminy procesowe.", "dr-03-prawo-procesowe", true, [], ["dr-03-prawo-procesowe"]);
        expect(selected.domainSkills).toEqual([
            "dr-03-prawo-procesowe"
        ]);
    });
    it("can select several cooperating execution skills automatically", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "analiza sadowa chronologia sprawy raport klienta ryzyka", "dr-03-prawo-procesowe", true, []);
        ...
        expect(selected.additionalSkills).toEqual(expect.arrayContaining([
            "analiza-sadowa-v6",
            "chronologia-sprawy-v1",
            "raport-klienta-v1"
        ]));
    ...
    it("does not activate execution skills merely because another skill mentions them in prose", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "analiza sadowa", "dr-03-prawo-procesowe", true, []);
        expect(selected.executionSkills).toContain("analiza-sadowa-v6");
    ...
    it("can add more than one legal domain to a single turn", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Pracownik pozywa pracodawcę; trzeba ocenić wypowiedzenie i przygotować pozew oraz terminy procesowe.", "dr-01-prawo-pracy", true, []);
    ...
    it("keeps manually selected execution and domain skills while auto-routing may add more", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Przygotuj analizę umowy i raport dla klienta.", "dr-02-prawo-cywilne", true, ["analizator-umow-v1", "dr-03-prawo-procesowe"]);
    ...
    it("routes an explicit simple-letter request before semantic scoring", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Przygotuj wezwanie do zapłaty.", "dr-02-prawo-cywilne", true, []);
    ...
    it("keeps process pleading priority when the query also mentions a simple pre-litigation letter", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Przygotuj pozew po bezskutecznym wezwaniu do zapłaty.", "dr-02-prawo-cywilne", true, []);
    ...
    it("normalizes Polish diacritics for explicit process routing", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Trzeba przygotować zażalenie na postanowienie.", "dr-03-prawo-procesowe", true, []);
        expect(selected.executionSkills).toContain("pisma-procesowe-v3");
    });
    it.each([
        [
    ...
    ])("routes explicit execution request %s to %s", (query, expectedSkill) => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, query, "dr-03-prawo-procesowe", true, []);
        expect(selected.executionSkills).toContain(expectedSkill);
        expect(selected.workflowExecutionSkill).toBe(expectedSkill);
    });
    it("routes explicit evidence material to the evidence analyzer", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Przeanalizuj nagranie, SMS-y i sprzeczności między dowodami.", "dr-03-prawo-procesowe", true, []);
    ...
    it("keeps a trivial chat command out of stateful legal workflows", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "napisz ok", "dr-01-prawo-pracy", true, []);
        expect(selected.executionSkills).toEqual([]);
        expect(selected.workflowExecutionSkill).toBeNull();
    });
    it("routes a KK article question only to the statute analyzer unless the user asks for another workflow", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Jaka jest odpowiedzialność karna z art. 276 kk?", "dr-03-prawo-procesowe", true, []);
        expect(selected.executionSkills).toEqual([
    ...
    it("routes a specific statutory interpretation request to the statute analyzer", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Zweryfikuj art. 415 KC i wyjaśnij przesłanki oraz wykładnię przepisu.", "dr-02-prawo-cywilne", true, []);
        expect(selected.executionSkills)
    ...
    it("uses the general legal guide when automatic mode has no semantic match", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "Nietypowe zagadnienie xyz bez charakterystycznych słów.", "dr-02-prawo-cywilne", true, []);
        expect(selected.executionSkills).toEqual([
            "przewodnik-prawny-v2"
        ]);
        expect(selected.workflowExecutionSkill).toBeNull();
    ...
    it("ignores unknown manual skill names when automatic mode is disabled", () => {
        const registry = registryWithSkills();
        const selected = resolveAdditionalSkills(registry, "zwykłe pytanie", "dr-02-prawo-cywilne", false, ["nie-istnieje", "shared"]);
