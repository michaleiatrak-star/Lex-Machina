...
function assertReadableResource(registry, skillName, semanticPath) {
    const resolved = registry.resolveResource(skillName, semanticPath);
    if (!resolved || !fs.statSync(resolved).isFile()) {
        throw new Error(`DETERMINISTIC_WORKFLOW_RESOURCE_MISSING:${semanticPath}`);
    }
    const stat = fs.statSync(resolved);
    if (stat.size < 1) {
        throw new Error(`DETERMINISTIC_WORKFLOW_RESOURCE_EMPTY:${semanticPath}`);
    }
}
export function createDeterministicWorkflowPlan(registry, workflowExecutionSkill) {
    const hasGuide = workflowExecutionSkill === "przewodnik-prawny-v2";
    const hasProcess = workflowExecutionSkill === "pisma-procesowe-v3";
    const hasSimple = workflowExecutionSkill === "pisma-proste-v2";
    const hasCourtAnalysis = workflowExecutionSkill === "analiza-sadowa-v6";
    const hasEvidenceAnalysis = workflowExecutionSkill === "analizator-dowodow-v3";
    const hasStatuteAnalysis = workflowExecutionSkill === "analizator-przepisow-v2";
    const hasContractAnalysis = workflowExecutionSkill === "analizator-umow-v1";
    const hasChronology = workflowExecutionSkill === "chronologia-sprawy-v1";
    const hasCaseLaw = workflowExecutionSkill === "orzeczenia-sadowe-v2";
    const hasWitnessQuestioning = workflowExecutionSkill === "przesluchanie-swiadkow-v2-min90";
    const hasClientReport = workflowExecutionSkill === "raport-klienta-v1";
    const hasSituationReport = workflowExecutionSkill === "raport-sytuacyjny-v2";
    ...
    if (executionSkill) {
        ...
        if (!skill) {
            throw new Error(`DETERMINISTIC_WORKFLOW_SKILL_MISSING:${executionSkill}`);
        }
        for (const resource of requiredFreshResources) {
            assertReadableResource(registry, executionSkill, resource);
        }
    }
    const requiredResourceSet = new Set(requiredFreshResources);
    const semanticContextResources = [
        ...(SEMANTIC_CONTEXT_RESOURCES[id] ?? [])
    ].filter((resource) => requiredResourceSet.has(resource));
    return {
        id,
        executionSkill,
...
export function deterministicWorkflowPrompt(plan) {
...
export function evaluateDeterministicWorkflowOutput(plan, text, context) {
    ...
    if (plan.id ===
        ...
        const observed = PROCESS_PLEADING_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = PROCESS_PLEADING_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = PROCESS_PLEADING_FINAL_MARKERS
    ...
    if (plan.id ===
        "COURT_ANALYSIS_V1") {
        const finalReport = context?.courtCheckpoint ===
            "FINAL_REPORT_PRESENTED";
        if (!finalReport) {
            return {
                workflow: plan.id,
                mode: "COURT_CHECKPOINT",
                required: [],
                observed: [],
                missing: [],
                orderValid: true,
        ...
        const observed = COURT_ANALYSIS_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = COURT_ANALYSIS_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = COURT_ANALYSIS_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    if (plan.id ===
        "EVIDENCE_ANALYSIS_V1") {
        if (context?.orderedCheckpoint &&
            context.orderedCheckpoint !==
                "AD-KROK4-DASHBOARD") {
            const prematureFinal = normalized.includes("RAPORT DOWODOWY");
            return {
                workflow: plan.id,
                mode: "EVIDENCE_CHECKPOINT",
                required: [],
                observed: prematureFinal
                    ? [
                        "PREMATURE_FINAL_REPORT"
                    ]
                    : [],
                missing: prematureFinal
                    ? [
                        "AD-KROK4-DASHBOARD_REQUIRED_FOR_FINAL_REPORT"
                    ]
                    : [],
                orderValid: !prematureFinal,
                result: prematureFinal
                    ? "BLOCKED"
                    : "PASS"
            };
        }
        const claimsFullReport = normalized.includes("RAPORT DOWODOWY");
        if (!claimsFullReport) {
            return {
                workflow: plan.id,
                mode: "NOT_APPLICABLE",
        ...
        const observed = EVIDENCE_ANALYSIS_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = EVIDENCE_ANALYSIS_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = EVIDENCE_ANALYSIS_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    if (plan.id ===
        "CONTRACT_ANALYSIS_V1") {
        const claimsLite = normalized.includes("RAPORT ANALIZY UMOWY — LITE");
        const claimsFull = !claimsLite &&
            normalized.includes("RAPORT ANALIZY UMOWY V1");
    ...
    if (plan.id ===
        "STATUTE_ANALYSIS_V1") {
        const claimsFullReport = normalized.includes("RAPORT ANALIZY");
        if (!claimsFullReport) {
            return {
                workflow: plan.id,
                mode: "NOT_APPLICABLE",
        ...
        const observed = STATUTE_ANALYSIS_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = STATUTE_ANALYSIS_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = STATUTE_ANALYSIS_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    if (plan.id ===
        ...
        if (context?.orderedCheckpoint &&
            context.orderedCheckpoint !==
                "W3-QUESTIONS") {
            const prematureW3 = normalized.includes("MACIERZ FINALNA") ||
                normalized.includes("SCORING FINALNY");
            return {
                workflow: plan.id,
                mode: "WITNESS_CHECKPOINT",
                required: [],
                observed: prematureW3
                    ? [
                        "PREMATURE_W3_FINAL"
                    ]
                    : [],
                missing: prematureW3
                    ...
                    : [],
                orderValid: !prematureW3,
                result: prematureW3
                    ? "BLOCKED"
                    : "PASS"
            };
        }
        const claimsFinalW3 = normalized.includes("ETAP W3") ||
            normalized.includes("MACIERZ FINALNA") ||
            normalized.includes("SCORING FINALNY");
        if (!claimsFinalW3) {
            return {
                workflow: plan.id,
        ...
        const observed = WITNESS_W3_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = WITNESS_W3_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = WITNESS_W3_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    if (plan.id ===
        "CHRONOLOGY_V1") {
        const claimsFullReport = normalized.includes("CHRONOLOGIA SPRAWY");
        if (!claimsFullReport) {
            return {
                workflow: plan.id,
                mode: "NOT_APPLICABLE",
        ...
        const observed = CHRONOLOGY_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = CHRONOLOGY_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = CHRONOLOGY_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    if (plan.id ===
        "CASE_LAW_V1") {
        const claimsFullReport = normalized.includes("RAPORT ORZECZEŃ");
        if (!claimsFullReport) {
            return {
                workflow: plan.id,
                mode: "NOT_APPLICABLE",
        ...
        const observed = CASE_LAW_FINAL_MARKERS
            .filter((marker) => normalized.includes(marker));
        const missing = CASE_LAW_FINAL_MARKERS
            .filter((marker) => !normalized.includes(marker));
        const positions = CASE_LAW_FINAL_MARKERS
            .map((marker) => normalized.indexOf(marker));
        const orderValid = missing.length === 0 &&
            positions.every((position, index) => index === 0 ||
                position >
    ...
    const intakeRequired = normalizedSimple.includes("DANE DO UZUPEŁNIENIA");
    const hasLetterBody = normalizedSimple.includes("TREŚĆ PISMA");
    if (intakeRequired &&
        !hasLetterBody) {
        const marker = "DANE DO UZUPEŁNIENIA";
        const markerIndex = normalizedSimple.indexOf(marker);
        const intakeTail = markerIndex >= 0
            ? normalizedSimple.slice(markerIndex +
                marker.length)
            : "";
        const hasSpecificMissingData = intakeTail
            .split(/\r?\n/)
            .some((line) => /[A-ZĄĆĘŁŃÓŚŹŻ0-9]{2,}/.test(line));
        const partialArtifactMarkers = SIMPLE_LETTER_READY_MARKERS
            .filter((readyMarker) => readyMarker !==
            "TREŚĆ PISMA" &&
            normalizedSimple.includes(readyMarker));
        const required = [
        ...
        return {
            ...
            observed,
            missing,
            orderValid: partialArtifactMarkers.length ===
                0,
            result: missing.length === 0
                ? "PASS"
                : "BLOCKED"
    ...
    const observed = SIMPLE_LETTER_READY_MARKERS
        .filter((marker) => normalizedSimple.includes(marker));
    const missing = SIMPLE_LETTER_READY_MARKERS
        .filter((marker) => !normalizedSimple.includes(marker));
    const positions = SIMPLE_LETTER_READY_MARKERS
        .map((marker) => normalizedSimple.indexOf(marker));
    const orderValid = missing.length === 0 &&
        positions.every((position, index) => index === 0 ||
            position >
                positions[index - 1]);
    const hasCompletionCount = normalizedSimple.includes("PISMO ZAWIERA") &&
        normalizedSimple.includes("PÓL DO UZUPEŁNIENIA");
    if (hasCompletionCount) {
        observed.push("PISMO ZAWIERA … PÓL DO UZUPEŁNIENIA");
    }
    else {
        missing.push("PISMO ZAWIERA … PÓL DO UZUPEŁNIENIA");
    }
    return {
        workflow: plan.id,
        mode: "READY_ARTIFACT",
...
export function evaluateDeterministicWorkflowReads(plan, corpusAudit, executionEvents = []) {
    const observed = [
        ...new Set([
            ...corpusAudit
                .filter((event) => event.tool === "read_legal_resource" &&
                event.decision === "ALLOW")
                .map((event) => event.target),
            ...executionEvents
                .filter((event) => event.type === "resource_read" &&
                event.status === "OK" &&
    ...
    const missing = plan.requiredFreshResources.filter((resource) => !observed.includes(resource));
    return {
        workflow: plan.id,
