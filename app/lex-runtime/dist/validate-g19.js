...
function currentFreshnessFetcher(stale = false) {
    const currentEli = stale
        ? "DU/2027/10"
        : "DU/2026/795";
    return async (input) => {
...
function historicalFreshnessFetcher() {
    return async (input) => {
        const url = String(input);
        if (url.endsWith("/DU/1964/93/references")) {
            return jsonResponse({
        ...
        throw new Error("Unexpected G19 historical URL: " +
            url);
    };
}
class TemporalProvider {
    asOf;
    id = "openai";
    label = "G19 deterministic temporal provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        ...
        if (!tool ||
            !readTool ||
            !params.runTools) {
            throw new Error("G19_REQUIRED_TOOL_MISSING");
        }
        ...
        if (readResults.length !==
            ...
            readResults.some((result) => typeof result.content !==
                "string" ||
                !result.content.trim())) {
            throw new Error("G19_STATUTE_PREFLIGHT_READ_FAILED");
        }
...
function appFor(registry, freshnessFetch, verificationFetches, asOf) {
    const providers = new ProviderRegistry();
    providers.register(new TemporalProvider(asOf));
    const verifier = new OfficialLegalSourceVerifier(async (input) => {
        verificationFetches.push(String(input));
    ...
    const freshness = new TemporalSourceFreshnessChecker(freshnessFetch, () => "2026-09-15T21:00:00.000Z");
...
function verificationSummary(body) {
...
const currentHttp = await request(appFor(registry, currentFreshnessFetcher(false), currentFetches))
    .post("/api/sessions/execute")
    .send(requestBody());
const staleFetches = [];
const staleHttp = await request(appFor(registry, currentFreshnessFetcher(true), staleFetches))
    .post("/api/sessions/execute")
    .send(requestBody());
const historicalFetches = [];
const historicalHttp = await request(appFor(registry, historicalFreshnessFetcher(), historicalFetches, "2020-06-01"))
    .post("/api/sessions/execute")
    .send(requestBody("2020-06-01"));
const afterRepealFetches = [];
const afterRepealHttp = await request(appFor(registry, historicalFreshnessFetcher(), afterRepealFetches, "2021-01-01"))
    .post("/api/sessions/execute")
    .send(requestBody("2021-01-01"));
const current = currentHttp.body;
const stale = staleHttp.body;
const historical = historicalHttp.body;
const afterRepeal = afterRepealHttp.body;
const currentVerification = verificationSummary(current);
const staleVerification = verificationSummary(stale);
const historicalVerification = verificationSummary(historical);
const afterRepealVerification = verificationSummary(afterRepeal);
const pass = issues.length === 0 &&
    ...
    typeof historical.answer ===
        "string" &&
    String(historical.answer).includes("STAN NA 2020-06-01") &&
    ...
    historicalFetches.every((url) => url ===
...
process.stdout.write(JSON.stringify({
    ...
    historicalRepealedTextException: {
        ...
        finalization: historical.finalization,
        markerContainsAsOf: typeof historical.answer === "string" &&
            String(historical.answer).includes("STAN NA 2020-06-01"),
        verification: historicalVerification,
        contentFetches: historicalFetches
    },
    historicalAfterRepeal: {
        asOf: "2021-01-01",
        http: afterRepealHttp.status,
        status: afterRepeal.status,
        answerReleased: "answer" in afterRepeal,
        verification: afterRepealVerification,
        contentFetches: afterRepealFetches
    },
