...
export function gateIWorkflowContract(workflow, executionSkill) {
    ...
    return {
        ...
        mandatoryPolicies: [...contract.mandatoryPolicies]
    };
}
export function evaluateGateIInputCompleteness(query, attachmentCount) {
    const attachmentAssertion = ATTACHMENT_ASSERTION.test(query);
    if (!attachmentAssertion) {
        return {
            result: "PASS",
            attachmentAssertion,
            attachmentCount,
            reason: "NO_ATTACHMENT_ASSERTION"
        };
    }
...
export function evaluateGateIWorkflowContract(args) {
    const checks = [
        {
            subgate: "I-I_INPUT_COMPLETENESS",
            result: args.input.result,
            detail: args.input.reason
        },
        {
            subgate: "I-J_STATE_TRANSITION",
            result: args.stateTransition,
            detail: `stateModel=${args.contract.stateModel}`
        }
    ];
    return {
        gate: "G39I_WORKFLOW_CONTRACT",
        workflow: args.contract.workflow,
        stateModel: args.contract.stateModel,
        commonInvariants: args.invariants.result,
        result: args.invariants.result ===
            "PASS" &&
            checks.every((check) => check.result !==
                "BLOCKED")
            ? "PASS"
            : "BLOCKED",
        checks
