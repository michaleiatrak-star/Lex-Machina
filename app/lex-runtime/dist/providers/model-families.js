...
function parseAnthropic(id) {
    const match = /^claude-(opus|sonnet|fable)-(\d+(?:-\d{1,2})*)(?:-\d{8})?$/i.exec(id);
    if (!match)
        return null;
    return {
...
function parseOpenAi(id) {
    const match = /^gpt-(\d+(?:\.\d+)?)(?:-(luna))?(?:-\d{4}-\d{2}-\d{2})?$/i.exec(id);
    if (!match)
        return null;
    return {
        family: "gpt",
...
function parseXai(id) {
    const match = /^grok-(\d+(?:\.\d+)?)(?:-\d{4})?$/i.exec(id);
    if (!match)
        return null;
    return {
        family: "grok",
 ...
 * they are.
 */
export function newestModelFamilies(models, perFamily = VERSIONS_PER_FAMILY) {
    const local = models.filter((model) => model.id.startsWith("local/"));
    const byVersion = new Map();
    for (const model of models) {
        if (model.id.startsWith("local/") || !model.selectable)
            continue;
    ...
    const chosen = [...families.values()].flatMap((entries) => entries
        .sort((a, b) => compareVersions(b.parsed.version, a.parsed.version))
        .slice(0, perFamily));
    const familyOrder = ["fable", "opus", "sonnet", "gpt", "grok"];
    chosen.sort((a, b) => familyOrder.indexOf(a.parsed.family) - familyOrder.indexOf(b.parsed.family) ||
 ...
 */
export const ACCOUNT_SESSION_MODELS = {
    anthropic: [
