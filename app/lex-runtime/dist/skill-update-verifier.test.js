...
import { verifySkillUpdateIndex } from "./skill-update-verifier.js";
function fixture() {
...
describe("signed skill update index", () => {
    it("accepts an Ed25519-signed compatible index", () => {
        ...
        const result = verifySkillUpdateIndex(indexBytes, signatureBytes, trusted);
    ...
    it("allows an unsigned index under the official-source SHA-256 policy", () => {
        ...
        const root = fs.mkdtempSync(path.join(os.tmpdir(), "lex-skill-unsigned-"));
        ...
        try {
            ...
            const result = verifySkillUpdateIndex(indexBytes, new Uint8Array(), undefined, manifestPath);
            expect(result.signerKeyId)
                .toBe("UNSIGNED_ALLOWED");
            expect(result.index.version)
                .toBe("0.1.4");
            expect(() => verifySkillUpdateIndex(Buffer.from("{}", "utf8"), new Uint8Array(), undefined, manifestPath)).toThrow("SKILL_UPDATE_INDEX_INVALID");
        }
        finally {
            fs.rmSync(root, {
                recursive: true,
    ...
    it("restores fail-closed behavior when signed mode is selected", () => {
        const { indexBytes } = fixture();
        const root = fs.mkdtempSync(path.join(os.tmpdir(), "lex-skill-signed-"));
        const manifestPath = path.join(root, "release-source.json");
        try {
            fs.writeFileSync(manifestPath, JSON.stringify({
                skillUpdate: {
            ...
            expect(() => verifySkillUpdateIndex(indexBytes, new Uint8Array(), undefined, manifestPath)).toThrow("SKILL_UPDATE_SIGNER_POLICY_MISSING");
        }
        finally {
            fs.rmSync(root, {
                recursive: true,
    ...
    it("rejects a tampered index even when the signature envelope is unchanged", () => {
        ...
        const tampered = Buffer.from(indexBytes
            .toString("utf8")
            .replace('"version":"0.1.4"', '"version":"0.1.5"'), "utf8");
        expect(() => verifySkillUpdateIndex(tampered, signatureBytes, trusted)).toThrow("SKILL_UPDATE_SIGNATURE_INVALID");
    });
    it("rejects a signature from an untrusted key id", () => {
        ...
        expect(() => verifySkillUpdateIndex(indexBytes, Buffer.from(JSON.stringify(envelope), "utf8"), trusted)).toThrow("SKILL_UPDATE_SIGNER_NOT_TRUSTED");
    });
    it("rejects an invalid index schema after a valid signature", () => {
        ...
        expect(() => verifySkillUpdateIndex(indexBytes, signatureBytes, trusted)).toThrow("SKILL_UPDATE_INDEX_INVALID");
    });
});
