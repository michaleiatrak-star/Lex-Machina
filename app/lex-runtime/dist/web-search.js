...
const USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) LexMachina/1.0";
export function assertPublicWebSearchQuery(raw) {
    const query = typeof raw === "string"
        ? raw.replace(/\s+/g, " ").trim()
        : "";
    if (!query ||
        query.length > MAX_QUERY_CHARS) {
        throw new WebSearchError("WEB_SEARCH_QUERY_INVALID");
    }
    ...
    if (/\[(?:LM)?PII:/iu.test(query) ||
        /\[(?:DOCUMENT|CASE KNOWLEDGE|FIRM KNOWLEDGE)\s/iu.test(query)) {
        throw new WebSearchError("WEB_SEARCH_CASE_DATA_FORBIDDEN");
    }
    return query;
}
...
export function parseDuckDuckGoHtml(html, maxResults) {
    ...
    while ((match = anchor.exec(html)) !== null) {
        ...
        if (classes.includes("result__a")) {
            ...
            const title = decodeEntities(body);
            if (title &&
                /^https?:\/\//i.test(url)) {
                hits.push({
                    title,
...
export class PublicWebSearch {
    ...
    fetchImpl;
    timeoutMs;
    constructor(env = process.env, fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS) {
        this.env = env;
        this.fetchImpl = fetchImpl;
        this.timeoutMs = timeoutMs;
    }
    async search(query, maxResults) {
        const provider = (this.env.LEX_WEB_SEARCH_PROVIDER ??
            "auto").trim().toLowerCase();
        if (provider === "off" ||
            provider === "disabled") {
            throw new WebSearchError("WEB_SEARCH_DISABLED");
        }
        const errors = [];
        const braveKey = this.env.BRAVE_SEARCH_API_KEY?.trim();
        if (braveKey &&
            (provider === "auto" || provider === "brave")) {
            try {
                const hits = await this.searchBrave(query, maxResults, braveKey);
                if (hits.length > 0) {
        ...
        if (provider === "auto" ||
            ...
            provider === "ddg") {
            try {
                const hits = await this.searchDuckDuckGo(query, maxResults);
                if (hits.length > 0) {
        ...
        throw new WebSearchError("WEB_SEARCH_NO_RESULTS", errors.join("; ") || `provider=${provider}`);
    }
    async searchBrave(query, maxResults, apiKey) {
    ...
    async searchDuckDuckGo(query, maxResults) {
        ...
        return parseDuckDuckGoHtml(await readLimitedText(response), maxResults);
    }
}
export function classifyWebSearchHits(hits) {
    return hits.map((hit) => {
        const sourceTier = classifyKnownLegalSourceUrl(hit.url) ?? "R3";
        return {
            ...hit,
            sourceTier,
            nextStep: sourceTier === "R1"
