...
async function satisfyCaseLawWorkflowPreflight(params, idPrefix) {
    ...
    if (!readTool || !params.runTools) {
        throw new Error("G24_CASE_LAW_PREFLIGHT_TOOL_MISSING");
    }
    ...
    for (const result of results) {
        ...
        if (payload.status !== "OK") {
            throw new Error("G24_CASE_LAW_PREFLIGHT_FAILED");
        }
    }
}
const signature = "III CZP 25/11";
const proposition = "SN wskazał na znaczenie tej zasady.";
const supportQuote = "pełny tekst orzeczenia zawiera tę dokładną wypowiedź";
function json(value) {
...
function caseLawDiscoveryFetcher(input) {
    ...
    return Promise.reject(new Error("G24_UNEXPECTED_DISCOVERY_URL:" +
        url));
}
...
class PropositionProvider {
    mode;
    id = "openai";
    label = "G24 deterministic proposition provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        await satisfyCaseLawWorkflowPreflight(params, "g24");
        if (this.mode ===
            "fake-support") {
            return {
                fullText: proposition +
                    " „" +
                    supportQuote +
                    "” sygn. III CZP 25/11 " +
        ...
        if (!tool ||
            !params.runTools) {
            throw new Error("G24_CASE_PROPOSITION_TOOL_MISSING");
        }
        ...
        const outputProposition = this.mode ===
            "altered-proposition"
            ? "SN przesądził tę zasadę."
            : proposition;
        const quotePart = this.mode ===
            "omit-support-quote"
            ? ""
            : " „" +
                supportQuote +
...
function appFor(registry, mode) {
    ...
    providers.register(new PropositionProvider(mode));
    ...
    const caseVerifier = new SupremeCourtCaseVerifier(caseFetcher(), () => "2026-09-15T23:45:00.000Z");
...
function body() {
    return {
        query: "Powiąż parafrazę z dokładnym dowodem z orzeczenia SN III CZP 25/11.",
        provider: "openai",
        model: "g24-model",
        primarySkill: DR02,
        mode: "PRAWNIK"
...
const supportedHttp = await request(appFor(registry, "supported"))
    .post("/api/sessions/execute")
    .send(body());
const fakeHttp = await request(appFor(registry, "fake-support"))
    .post("/api/sessions/execute")
    .send(body());
const alteredHttp = await request(appFor(registry, "altered-proposition"))
    .post("/api/sessions/execute")
    .send(body());
const omittedHttp = await request(appFor(registry, "omit-support-quote"))
    .post("/api/sessions/execute")
    .send(body());
const supported = supportedHttp.body;
const fake = fakeHttp.body;
const altered = alteredHttp.body;
const omitted = omittedHttp.body;
const supportedSummary = verification(supported);
const fakeSummary = verification(fake);
const alteredSummary = verification(altered);
const omittedSummary = verification(omitted);
const pass = issues.length === 0 &&
    ...
    typeof supported.answer ===
        "string" &&
    String(supported.answer).includes(proposition) &&
    String(supported.answer).includes(supportQuote) &&
    String(supported.answer).includes("CASE-SUPPORT:") &&
...
process.stdout.write(JSON.stringify({
    gate: "G24_SN_PROPOSITION_EVIDENCE_LINK",
    result: pass
        ? "PASS"
        : "BLOCKED",
    supportedPath: {
        http: supportedHttp.status,
        status: supported.status,
        finalization: supported.finalization,
        verification: supportedSummary,
        answerReleased: typeof supported.answer ===
            "string",
    ...
    omittedSupportQuote: {
        http: omittedHttp.status,
        status: omitted.status,
        verification: omittedSummary,
        answerReleased: "answer" in omitted
    },
