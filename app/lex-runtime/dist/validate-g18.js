...
import { DeterministicLegalActResolver } from "./legal-act-resolver.js";
...
const EXPECTED_URL = "https://api.sejm.gov.pl/eli/acts/DU/2026/795/text.html";
...
function g18FreshnessFetcher(input) {
    ...
    return Promise.reject(new Error("G18_UNEXPECTED_FRESHNESS_URL:" +
        url));
}
class ResolverProvider {
    id = "openai";
    label = "G18 deterministic resolver provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        const tool = params.tools?.find((candidate) => candidate.function.name === "verify_legal_reference");
        const readTool = params.tools?.find((candidate) => candidate.function.name === "read_legal_resource");
        if (!tool ||
            !readTool ||
            !params.runTools) {
            throw new Error("G18_REQUIRED_TOOL_MISSING");
        }
        ...
        if (readResults.length !==
            ...
            readResults.some((result) => typeof result.content !==
                "string" ||
                !result.content.trim())) {
            throw new Error("G18_STATUTE_PREFLIGHT_READ_FAILED");
        }
        const parameters = tool.function.parameters;
        if (parameters.properties?.url ||
        ...
        return {
            fullText: "Znaczenie ma art. 5 KC. " + payload.marker
        };
    }
}
const fetchInputs = [];
const verifier = new OfficialLegalSourceVerifier(async (input) => {
    ...
    return new Response("<html><head><title>Kodeks cywilny</title></head>" +
...
const kpk = resolver.resolve("Kodeks postępowania karnego");
const unknownLedger = new VerificationLedger();
const unknownRuntime = new LegalVerificationToolRuntime(unknownLedger, verifier, resolver);
...
const unknownPayload = JSON.parse(unknownResult?.content ?? "{}");
const providers = new ProviderRegistry();
providers.register(new ResolverProvider());
const sessionExecutor = new SafeSessionExecutor(registry, new ProviderGateway(providers), undefined, (ledger) => new LegalVerificationToolRuntime(ledger, verifier, resolver, new TemporalSourceFreshnessChecker(g18FreshnessFetcher, () => "2026-09-15T20:00:00.000Z")));
...
const response = await request(app)
    ...
    .send({
    query: "Zweryfikuj art. 5 KC bez podawania adresu źródła przez model.",
    provider: "openai",
    model: "g18-model",
    primarySkill: DR02,
    mode: "PRAWNIK"
...
const pass = issues.length === 0 &&
    ...
    fetchInputs.every((input) => input === EXPECTED_URL);
