...
export function classifyKnownLegalSourceUrl(value) {
...
function olderThanMonths(value, months, now) {
    const parsed = new Date(value);
    if (Number.isNaN(parsed.getTime())) {
        return true;
    }
    const threshold = new Date(now.getTime());
    threshold.setUTCMonth(threshold.getUTCMonth() -
        months);
    return (parsed.getTime() <
        threshold.getTime());
}
export function assessLegalSourceCandidate(candidate, now = new Date()) {
    const auxiliaryOnly = candidate.tier ===
        "R2B" ||
        candidate.tier ===
            "R3";
    const requiresHigherTierCrossCheck = auxiliaryOnly;
    const higherTierCrossCheckSatisfied = candidate.crossCheckStatus ===
        "CONFIRMED_R1_R2A" &&
        (candidate.crossCheckTier ===
            "R1" ||
            candidate.crossCheckTier ===
                "R2A") &&
        Boolean(candidate.crossCheckUrl);
    const conflict = candidate.crossCheckStatus ===
        "CONFLICT";
    const date = sourceDate(candidate);
    const staleOrUndatedWarning = candidate.tier ===
        "R3" &&
        (!date ||
            olderThanMonths(date, 24, now));
    if (auxiliaryOnly) {
        return {
            tier: candidate.tier,
            auxiliaryOnly: true,
            canBeSoleLegalBasis: false,
            canCreateVerifiedMarker: false,
            requiresHigherTierCrossCheck: true,
    ...
    return {
        tier: candidate.tier,
        auxiliaryOnly: false,
        canBeSoleLegalBasis: true,
        canCreateVerifiedMarker: candidate.provenance
            .retrievedVia ===
            "LEX_NATIVE" &&
            candidate.provenance
                .accessMode ===
                "DIRECT_LIVE",
        requiresHigherTierCrossCheck: false,
        higherTierCrossCheckSatisfied: true,
        staleOrUndatedWarning: false,
        conflict,
        instruction: "R1/R2A material may participate in the native verification path. A source tier alone never creates VERIFIED; the applicable Lex native verifier must still succeed."
    };
}
export function assertVerificationTierPolicy(input) {
    if ((input.status ===
        "VERIFIED" ||
        input.status ===
            "SUPPORTED") &&
        (input.sourceTier ===
            ...
            input.sourceTier ===
                "R3")) {
        throw new Error("AUXILIARY_SOURCE_CANNOT_CREATE_VERIFIED_OR_SUPPORTED_STATUS");
    }
}
const FEDERATED_POLICIES = {
    ...
    krs: {
        ...
        verificationEligible: false,
        crossCheckRequired: true,
        note: "Official registry material may establish registry facts, not statutory wording."
    },
    eureka: {
        sourceTier: "R2A",
        provenance: "EXTERNAL_MCP",
    ...
    "eu-sparql": {
        ...
        verificationAuthority: "LEX_NATIVE_ONLY",
        verificationEligible: false,
        requiresDocumentTierClassification: true,
        crossCheckRequired: true,
...
export function federatedSourcePolicy(sourceId) {
    return (FEDERATED_POLICIES[sourceId] ?? {
        sourceTier: "R3",
        provenance: "UNKNOWN",
        transport: "FEDERATED_MCP",
        verificationAuthority: "LEX_NATIVE_ONLY",
        verificationEligible: false,
        crossCheckRequired: true,
        note: "Unknown federated source is treated conservatively as R3 research-only material until explicitly classified."
    });
}
