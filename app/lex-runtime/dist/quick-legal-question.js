...
function articleRefs(text) {
    return new Set([...text.matchAll(/\bart\.?\s*(\d{1,3}[a-z]?)/giu)].map((match) => match[1].toLowerCase()));
}
function qualifierSections(partsDirectory) {
    let files;
    try {
        files = fs
 ...
 */
export function criminalQualifierExcerpt(partsDirectory, question, maxChars = 4_200) {
    const queryStems = stems(question);
    const queryArticles = articleRefs(question);
    if (queryStems.size === 0 && queryArticles.size === 0)
        return null;
    const scored = qualifierSections(partsDirectory)
        ...
        for (const stem of queryStems) {
            if (headingStems.has(stem))
                score += 3;
            else if (sectionStems.has(stem))
                score += 1;
        }
        for (const article of articleRefs(section.text)) {
            if (queryArticles.has(article))
                score += 3;
        }
        // Tree nodes answer qualification questions; long reference tables
        // and audit notes do not.
        if (/DRZEWO|WĘZEŁ|WEZEL|TABELA POR/iu.test(section.heading))
            score += 1;
    ...
    for (const { section } of scored) {
        const text = section.text.length > 2_600
...
export function qualifierPrinciples(indexText) {
    const principle = /## ZASADA NACZELNA([\s\S]*?)\n---/u.exec(indexText)?.[1]?.trim() ?? "";
    return [
        principle,
