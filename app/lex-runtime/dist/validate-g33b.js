...
const checks = {
    currentUserInstaller: config.bundle?.windows?.nsis?.installMode === "currentUser",
    webViewDownloadIfMissing: config.bundle?.windows?.webviewInstallMode?.type === "downloadBootstrapper",
    thinBundledRuntime: config.bundle?.resources === undefined &&
        hooks.includes('SetOutPath "$INSTDIR\\runtime"') &&
        hooks.includes('File /r "${LEX_HOOK_FILE_DIR}\\..\\runtime\\*"') &&
        hooks.includes('IfFileExists "$INSTDIR\\runtime\\app\\dist\\http\\server.js"') &&
    ...
    privateNodeDownloadIfMissing: bootstrap.includes("Test-CommandVersion") &&
    ...
    privatePythonDownloadIfMissing: source.runtime?.python?.delivery === "EMBEDDABLE_APP_LOCAL" &&
    ...
    pinnedPackagesOnlyIfNeeded: bootstrap.includes("verify-python-package-set.py") &&
    ...
    modelsDownloadIfMissing: bootstrap.includes("prefetch-release-models.py") &&
    ...
    visualCppDownloadIfMissing: source.systemPrerequisites?.visualCppRuntime?.delivery === "DOWNLOAD_IF_MISSING" &&
        /^[a-f0-9]{64}$/i.test(source.systemPrerequisites?.visualCppRuntime?.sha256 ?? "") &&
        bootstrap.includes("visual-cpp-runtime") &&
        bootstrap.includes('Get-VerifiedDownload $vc.url $vc.sha256 $vcInstaller "visual-cpp-runtime"') &&
        bootstrap.includes("if (-not (Test-IsAdministrator))") &&
        bootstrap.includes('$startArgs.Verb = "RunAs"'),
    verifiedSourceCache: source.notes?.cachePolicy === "REUSE_ONLY_AFTER_SHA256_VERIFICATION" &&
        bootstrap.includes("Using verified cache") &&
        bootstrap.includes("BOOTSTRAP_HASH_MISMATCH"),
    coreRuntimeNetworkIndependentAfterBootstrap: source.localAi?.delivery === "USER_INITIATED_AFTER_INSTALL" &&
    ...
    postInstallBootstrapFailClosed: hooks.includes("windows-online-bootstrap.ps1") &&
