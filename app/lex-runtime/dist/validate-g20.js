...
const freshnessFetch = async (input) => {
    ...
    throw new Error("Unexpected G20 freshness URL: " +
        url);
};
class PdfProvider {
    id = "openai";
    label = "G20 deterministic PDF provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        ...
        if (!tool ||
            !readTool ||
            !params.runTools) {
            throw new Error("G20_REQUIRED_TOOL_MISSING");
        }
        ...
        if (readResults.length !==
            ...
            readResults.some((result) => typeof result.content !==
                "string" ||
                !result.content.trim())) {
            throw new Error("G20_STATUTE_PREFLIGHT_READ_FAILED");
        }
...
function appFor(registry, extractor, pdfFetches) {
    const providers = new ProviderRegistry();
    providers.register(new PdfProvider());
    const verifier = new OfficialLegalSourceVerifier(async (input) => {
        pdfFetches.push(String(input));
    ...
    }, () => "2026-09-15T22:00:00.000Z", extractor);
    const freshness = new TemporalSourceFreshnessChecker(freshnessFetch, () => "2026-09-15T22:00:00.000Z");
...
function requestBody() {
    return {
        query: "Zweryfikuj art. 5 KC z urzędowego tekstu PDF.",
        provider: "openai",
        model: "g20-model",
        primarySkill: DR02,
        mode: "PRAWNIK"
...
const goodFetches = [];
const goodHttp = await request(appFor(registry, goodExtractor, goodFetches))
    .post("/api/sessions/execute")
    .send(requestBody());
const badExtractor = {
    async extract() {
        throw new PdfTextExtractionError("No extractable text.", "PDF_NO_TEXT");
    }
};
const badFetches = [];
const badHttp = await request(appFor(registry, badExtractor, badFetches))
    .post("/api/sessions/execute")
    .send(requestBody());
const good = goodHttp.body;
const bad = badHttp.body;
const goodVerification = verificationSummary(good);
const badVerification = verificationSummary(bad);
const expectedPdfUrl = "https://api.sejm.gov.pl/eli/acts/DU/2026/795/text.pdf";
const pass = issues.length === 0 &&
    ...
    typeof good.answer ===
        "string" &&
    String(good.answer).includes(expectedPdfUrl) &&
    ...
    goodFetches.every((url) => url === expectedPdfUrl) &&
    badHttp.status === 200 &&
    bad.status === "DRAFT_PRESENTABLE" &&
    typeof bad.answer === "string" &&
    badVerification.records === 0 &&
    badFetches.length >= 1 &&
    badFetches.every((url) => url === expectedPdfUrl);
process.stdout.write(JSON.stringify({
    gate: "G20_OFFICIAL_PDF_VERIFICATION",
    result: pass
        ? "PASS"
        : "BLOCKED",
    verifiedPdfPath: {
        ...
        sourceUrl: goodFetches[0] ?? null,
        answerReleased: typeof good.answer ===
            "string",
        loadedSkills: good.loadedSkills,
        workflow: good.workflow,
        gateI: good.gateI,
        gateITurn: good.gateITurn
    },
    extractionFailurePath: {
        http: badHttp.status,
        status: bad.status,
        finalization: bad.finalization,
        verification: badVerification,
        sourceUrl: badFetches[0] ?? null,
        answerReleased: "answer" in bad,
        loadedSkills: bad.loadedSkills,
        workflow: bad.workflow,
        gateI: bad.gateI,
        gateITurn: bad.gateITurn
    },
