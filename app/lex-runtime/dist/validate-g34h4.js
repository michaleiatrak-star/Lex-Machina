...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "lex-g34h4-validate-"));
...
try {
    ...
    const vault = new EncryptedPrivacyVaultStore({
        rootDir: root
    });
    const uploads = new SecureCaseUploadStore({
        rootDir: root
    });
    const documents = new SecureCaseDocumentStore({
        rootDir: root
    });
    const artifacts = new SecureCaseArtifactStore({
        rootDir: root
    });
    const rotation = new CaseSecurityRotationCoordinator(vault, uploads, documents, artifacts);
    const cases = new LocalCaseAccessService(authStore, auth, files, rotation);
    const owner = await auth.bootstrap({
        loginName: "g34h4-owner",
        displayName: "G34H4 Owner",
        password: "G34H4 walidacyjne bezpieczne haslo 2026"
    });
    const actor = {
        user: owner.user,
        session: owner.session
    };
    const legalCase = await cases.createCase(actor, "G34H4");
    const clear = Buffer.from("Jan Kowalski PESEL 44051401458 — finalny artefakt");
    const filename = "Pozew Jan Kowalski.docx";
    let artifactId = "";
    ...
    const artifactRoot = path.join(root, "cases", legalCase.caseId, "secure", "artifacts");
    const disk = allBytes(artifactRoot);
    const noPlaintext = !disk.includes(Buffer.from(filename, "utf8")) &&
        !disk.includes(Buffer.from("Jan Kowalski", "utf8")) &&
        !disk.includes(Buffer.from("44051401458", "utf8"));
    let roundTrip = false;
    await cases.withCaseDataKey(actor, legalCase.caseId, "READ", async (caseDataKey) => {
        const restored = await artifacts
            .readArtifact({
            caseId: legalCase.caseId,
            artifactId,
            caseDataKey,
    ...
    process.stdout.write(JSON.stringify({
        gate: "G34H4_ENCRYPTED_ARTIFACT_STORE",
        result: pass
            ? "PASS"
            : "BLOCKED",
        artifactId,
        clearArtifactOrFilenamePersisted: !noPlaintext,
        encryptedRoundTrip: roundTrip,
        caseKeyRotationCompatible: rotatedRoundTrip,
        fullG34HClaimed: false,
        remaining: [
            "G34H5_LEGACY_MIGRATION"
        ],
        generationIntegration: "PENDING_G31D_G31E"
    }, null, 2) +
        "\n");
    clear.fill(0);
    auth.close();
