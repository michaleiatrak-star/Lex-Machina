...
const root = await mkdtemp(path.join(os.tmpdir(), "lex-g31b-"));
function makeZip(target, unsafe) {
...
`;
    const run = spawnSync(process.env.LEX_STORAGE_PYTHON ??
        "python3", [
        "-c",
        script,
        target,
...
try {
    ...
    const incoming = await readdir(path.join(root, "cases", localCase.caseId, "incoming"));
    const pass = safe.archive === true &&
        safe.extracted.some((entry) => entry.relativePath ===
            "akta/readme.txt") &&
        safe.extracted.some((entry) => entry.relativePath ===
            "scan.pdf" &&
            entry.processable) &&
    ...
    process.stdout.write(JSON.stringify({
        ...
        result: pass
            ? "PASS"
            : "BLOCKED",
        extractedEntries: safe.extracted.length,
        traversalBlocked,
        rejectedUploadRemoved: incoming.length === 1,
        recursiveArchiveExtraction: false
    }, null, 2) + "\n");
    if (!pass)
        process.exitCode = 1;
}
