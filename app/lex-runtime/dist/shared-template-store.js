...
function validateMediaType(mediaType, filename) {
    ...
    throw new Error("UNSUPPORTED_TEMPLATE_MEDIA_TYPE");
}
export class LocalSharedTemplateStore {
    ...
    constructor(options = {}) {
        this.rootDir = path.resolve(options.rootDir ??
            defaultRootDir());
        this.templatesDir =
            path.join(this.rootDir, "shared", "templates");
        this.maxBytes =
            options.maxBytes ??
                64 * 1024 * 1024;
    }
    templateDir(templateId) {
        ...
        const target = path.resolve(this.templatesDir, templateId);
        const base = path.resolve(this.templatesDir) + path.sep;
    ...
    async saveTemplate(args) {
        if (!/^user_[a-f0-9]{32}$/
            ...
            throw new Error("INVALID_TEMPLATE_CREATOR");
        }
        if (args.data.byteLength < 1 ||
            args.data.byteLength >
                this.maxBytes) {
            throw new Error("TEMPLATE_SIZE_INVALID");
        }
        const filename = safeFilename(args.filename);
        const mediaType = validateMediaType(args.mediaType, filename);
        ...
        await writeFile(partialPath, args.data, {
            flag: "wx",
            mode: 0o600
        });
    ...
    async readTemplate(templateId) {
        ...
        const data = await readFile(path.join(dir, "original", manifest.filename));
        ...
        if (data.byteLength !==
            ...
            sha256 !==
                manifest.sha256) {
            data.fill(0);
            throw new Error("TEMPLATE_PAYLOAD_MISMATCH");
        }
        return {
            manifest,
            data
    ...
    async listTemplates() {
        let entries;
        try {
            entries = await readdir(this.templatesDir, {
                withFileTypes: true
            });
        }
        catch (error) {
        ...
        for (const entry of entries) {
            ...
            try {
                const manifest = JSON.parse(await readFile(path.join(this.templateDir(entry.name), "manifest.json"), "utf8"));
                if (manifest.templateId !==
                    entry.name ||
                    manifest.scope !==
                        "FIRM_SHARED" ||
