...
export function documentIdFromSha256(sha256) {
...
export async function purgeSecureCaseDocument(args) {
    ...
    const caseRoot = path.resolve(args.rootDir, "cases", args.caseId);
    const documentsRoot = path.resolve(caseRoot, "secure", "documents");
    const target = path.resolve(documentsRoot, args.documentId);
