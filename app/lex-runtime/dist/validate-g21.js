...
function amendmentFixture(mode) {
    if (mode === "future") {
        return {
            eli: "DU/2026/1001",
    ...
    if (mode === "effective") {
        return {
            eli: "DU/2026/1002",
            relationDate: "2026-09-01",
            promulgation: "2026-07-15",
            entryIntoForce: "2026-09-01"
        };
    }
    return {
        eli: "DU/2026/1003",
...
function freshnessFetcher(mode) {
    const amendment = amendmentFixture(mode);
    return async (input) => {
                ...
                "Akty zmieniające": [{
                        act: {
                            ...
                            ...(amendment.relationDate
                                ? {
                                    date: amendment.relationDate
                                }
                                : {}),
                            promulgation: amendment.promulgation,
                            displayAddress: amendment.eli,
                            title: "Ustawa zmieniająca"
                        }
                    }]
            });
        }
        if (url.endsWith("/DU/2026/795/references")) {
            return jsonResponse({
        ...
        if (url.endsWith("/" +
            amendment.eli +
            "/text.html")) {
            return new Response("<html><body><p>Art. 1. W art. 6 Kodeksu cywilnego wprowadza się zmianę.</p></body></html>", {
                status: 200,
                headers: {
        ...
        if (url.endsWith("/" + amendment.eli)) {
            return jsonResponse({
                ...
                ...(amendment.entryIntoForce
                    ? {
                        entryIntoForce: amendment.entryIntoForce
                    }
                    : {})
            });
        }
        throw new Error("Unexpected G21 ELI URL: " +
            url);
    };
}
class AmendmentProvider {
    id = "openai";
    label = "G21 deterministic amendment provider";
    capabilities = {
        streaming: true,
        tools: true,
        reasoning: true,
    ...
    async stream(params) {
        ...
        if (!tool ||
            !readTool ||
            !params.runTools) {
            throw new Error("G21_REQUIRED_TOOL_MISSING");
        }
        ...
        const readResults = await params.runTools(statuteResources.map((resource, index) => ({
            id: "g21-read-" +
                String(index + 1),
        ...
        if (readResults.length !==
            ...
            readResults.some((result) => typeof result.content !==
                "string" ||
                !result.content.trim())) {
            throw new Error("G21_STATUTE_PREFLIGHT_READ_FAILED");
        }
        ...
        if (payload.status === "VERIFIED" &&
            typeof payload.marker ===
                "string") {
            return {
                fullText: "Znaczenie ma art. 5 KC. " +
                    payload.marker
            };
        }
        return {
...
function appFor(registry, mode, contentFetches) {
    const providers = new ProviderRegistry();
    providers.register(new AmendmentProvider());
    const verifier = new OfficialLegalSourceVerifier(async (input) => {
        contentFetches.push(String(input));
        return new Response("<html><title>Kodeks cywilny</title>" +
    ...
    const freshness = new TemporalSourceFreshnessChecker(freshnessFetcher(mode), () => "2026-09-15T23:00:00.000Z");
    return createLexHttpApp({
        ...
        modelCatalog: {
            list: async () => [{
                    provider: "openai",
                    id: "g21-model",
                    displayName: "g21-model",
                    selectable: true
                }]
        },
        sessionExecutor: new SafeSessionExecutor(registry, new ProviderGateway(providers), undefined, (ledger) => new LegalVerificationToolRuntime(ledger, verifier, undefined, freshness))
    });
}
function body() {
    return {
        query: "Zweryfikuj art. 5 KC z uwzględnieniem nowelizacji po tekście jednolitym.",
        provider: "openai",
        model: "g21-model",
        primarySkill: DR02,
        mode: "PRAWNIK"
...
const futureFetches = [];
const futureHttp = await request(appFor(registry, "future", futureFetches))
    .post("/api/sessions/execute")
    .send(body());
const effectiveFetches = [];
const effectiveHttp = await request(appFor(registry, "effective", effectiveFetches))
    .post("/api/sessions/execute")
    .send(body());
const unknownFetches = [];
const unknownHttp = await request(appFor(registry, "unknown", unknownFetches))
    .post("/api/sessions/execute")
    .send(body());
const future = futureHttp.body;
const effective = effectiveHttp.body;
const unknown = unknownHttp.body;
const futureVerification = summary(future);
const effectiveVerification = summary(effective);
const unknownVerification = summary(unknown);
const pass = issues.length === 0 &&
    ...
    effectiveFetches.every((url) => url ===
...
process.stdout.write(JSON.stringify({
    gate: "G21_AMENDMENT_AWARE_LEGAL_STATE",
    result: pass ? "PASS" : "BLOCKED",
    futureAmendment: {
        http: futureHttp.status,
        status: future.status,
        finalization: future.finalization,
        verification: futureVerification,
        contentFetches: futureFetches.length,
        allowedBecause: "amendment effective date is after target legal-state date"
    },
    effectiveAmendment: {
        http: effectiveHttp.status,
        status: effective.status,
        answerReleased: "answer" in effective,
        verification: effectiveVerification,
        contentFetches: effectiveFetches.length,
        allowedBecause: "official amendment text proves the requested article is untouched; verification uses the latest consolidated text"
    },
    unknownEffectDate: {
        http: unknownHttp.status,
        status: unknown.status,
        answerReleased: "answer" in unknown,
        verification: unknownVerification,
        contentFetches: unknownFetches.length,
        degradedBecause: "official amendment effect date could not be established"
    },
