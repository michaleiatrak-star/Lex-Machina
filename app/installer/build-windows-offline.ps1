...
Write-Host "[1/11] Build runtime JS"
...
Write-Host "[2/11] Copy workers, corpus and bootstrap"
...
Write-Host "[4/11] Private Python"
...
Write-Host "[5/11] Pinned Python/ML packages"
...
Write-Host "[6/11] Prefetch OCR/NER models"
...
Write-Host "[7/11] Prefetch verified local LLM runtime and models"
& (Join-Path $installer "install-local-llm.ps1") `
  -RuntimeRoot $payload `
  -ManifestPath (Join-Path $payload "release-source.json") `
  -CacheRoot (Join-Path $cache "local-llm")
if ($LASTEXITCODE -ne 0) { throw "Local LLM prefetch failed" }

Write-Host "[8/11] Bundle Visual C++ runtime prerequisite"
...
Write-Host "[9/11] Build native runtime sidecar"
...
Write-Host "[10/11] Generate immutable component lock"
...
Write-Host "[11/11] Offline self-test"
