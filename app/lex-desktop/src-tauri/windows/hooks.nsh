...
lex_online_bootstrap:
  ...
  DetailPrint "Ten etap pobiera i konfiguruje składniki. Może potrwać kilka minut."
  DetailPrint "Instalator działa — kolejne etapy i komunikaty będą wyświetlane poniżej."
  nsExec::ExecToLog '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$INSTDIR\runtime\bootstrap\windows-online-bootstrap-entry.ps1" -RuntimeRoot "$INSTDIR\runtime"'
  Pop $0
  ${If} $0 != 0
    FileOpen $2 "$INSTDIR\runtime\bootstrap-install-error.log" w
    FileWrite $2 "stage=online-bootstrap-entry$\r$\nexit=$0$\r$\nlog=$INSTDIR\runtime\bootstrap-install.log$\r$\n"
    FileClose $2
    MessageBox MB_ICONSTOP|MB_OK "Lex Machina: przygotowanie składników nie powiodło się.$\r$\nKod: $0$\r$\nSzczegóły: $INSTDIR\runtime\bootstrap-install.log" /SD IDOK
    Abort
  ${EndIf}
